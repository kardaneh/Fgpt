...
from typing import Generator, Dict, List, Set, Tuple, Optional, Any, Iterable
    ...
    * Preserves names recorded in :attr:`_local_scope` to avoid replacing
    ...
    def get_attr_node(self, name: str) -> ast.Attribute | None:
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in get_attr_node of \
                                ReplaceGlobals') from e
    ...
    def visit_Name(self, node: ast.Name) -> ast.AST:
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_Name of \
                                ReplaceGlobals: {ast.dump(node,indent=4)}') from e
    ...
    def visit_Attribute(self, node):
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_Attribute of \
                                ReplaceGlobals: {ast.dump(node,indent=4)}') from e
    ...
    def visit_Assign(self, node):
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_Assign of \
                                ReplaceGlobals: {ast.dump(node,indent=4)}') from e
    ...
    def visit_Call(self,node):
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_Call of \
                                ReplaceGlobals: {ast.dump(node,indent=4)}') from e
    ...
    def visit_For(self,node):
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_For of \
                                ReplaceGlobals: {ast.dump(node,indent=4)}') from e
    ...
    def visit_If(self, node):
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_If of \
                                ReplaceGlobals: {ast.dump(node,indent=4)}') from e
    ...
    def _replace_compare(self, compare_node: ast.Compare) -> None:
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in _replace_compare of \
                                ReplaceGlobals: {ast.dump(compare_node,indent=4)}') from e
    ...
    def visit_BinOp(self, node: ast.BinOp) -> ast.BinOp:
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_BinOp \
                               of ReplaceGlobals: {ast.dump(node,indent=4)}') from e
    ...
    **kwargs
    ...
    def visit_Subscript(self, node: ast.Subscript) -> ast.Subscript:
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_Subscript of AdjustIndices: {ast.dump(node,indent=4)}') from e
        ...
        try:
            ...
            if isinstance(node.targets[0], (ast.Name,ast.Attribute)):
                ...
                if isinstance(node.targets[0].value,(ast.Name, ast.Attribute)):
                    name = node.targets[0].value.id if isinstance(node.targets[0].value,ast.Name) \
            ...
            raise RuntimeError(f'RuntimeError in visit_Assign \
                               of AdjustIndices: {ast.dump(node,indent=4)}') from e
    ...
    def visit_For(self, node: ast.For) -> ast.For:
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_For \
                               of AdjustIndices: {ast.dump(node,indent=4)}') from e
    ...
    def _handle_adjusted_left(self, binop_node: ast.BinOp, parent_node: ast.AST) -> ast.AST:
        """
    ...
    def visit_If(self, node: ast.If) -> ast.If:
        ...
        try:
            ...
            raise RuntimeError(f'RuntimeError in visit_For of AdjustIndices: {ast.dump(node,indent=4)}') from e
        ...
        try:
            ...
            if isinstance(node.left, ast.Name) and (node.left.id in self.CONV_VARS or node.left.id in self.adjusted_vars):
                ...
                if isinstance(node.left.value, ast.Name) and \
                    (node.left.value.id in self.CONV_VARS or node.left.value.id in self.adjusted_vars):
                    for i in range(len(node.comparators)):
                        if isinstance(node.comparators[i], ast.Subscript) \
        ...
        try:
            ...
            if isinstance(node.func, ast.Attribute) and node.func.attr in ['argmin','argmax']:
                ...
                if subscript_nodes:
                    ...
                    arr_name = subscript_node.value.id if isinstance(subscript_node.value, ast.Name)  \
    ...
    def _check_internal_call_element(self, node: ast.AST) -> ast.AST:
        ...
        try:
            ...
            int_call = (isinstance(node, ast.Call) 
            ...
            if int_call:
                ...
                if len(node.args) == 1:
                    ...
                    if isinstance(arg, ast.Subscript) and isinstance(arg.value, ast.Name):
                        ...
                        if var_name not in self.adjusted_vars \
                            and ((not self.exclude_index) or (var_name not in self.exclude_index)):
                            return True
                    else:
            ...
            raise RuntimeError(f"_check_internal_call_element failed during the transformation: node={ast.dump(node,indent=4)}") from e
        ...
        try:
            if isinstance(rhs, ast.Name) and rhs.id not in self.CONV_VARS:
                ...
                if isinstance(rhs.op, ast.Sub):
                    return ast.BinOp(left=rhs.left, op=ast.Sub(), 
    ...
    def _handle_binop(self, node: ast.BinOp) -> ast.AST:
        ...
        try:
            self.generic_visit(node)
            check = (isinstance(node.left, ast.Name) 
...
def attach_instance(node: ast.AST, instance_name: Optional[str] = 'self'):
    """
...
def safe_eval_expr(node, attributes: Optional[Dict[str, Tuple]] = None) -> Any:
    """
