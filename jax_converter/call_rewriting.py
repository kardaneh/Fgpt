...
from jax_utils import get_name, contains_name, collect_reads_before_def
...
class _CallRewriting:
    ...
    def visit_Call(self, node: ast.Call) -> ast.AST:
        ...
        * active loop metadata
        ...
        try:
            ...
            needs_vmap = False
            in_axes = []
            if isinstance(node.func, ast.Attribute) and node.func.attr in callees:
                loop_info = vectorization_context.get('loop_info', {}) if vectorization_context else {}
                        ...
                        for arg_name, actual_shape in edge.arg_shapes.items():
                            ...
                            if len(actual_shape) > len(expected_shape):
                                ...
                                for i, sh in enumerate(actual_shape):
                                    if sh in loop_info and len(in_axes) < len(node.args):
                                        in_axes.append(i)
                        ...
                        vmap_call = ast.Call(
                            func=ast.Call(
                                ...
                                keywords=[ast.keyword(
                                    arg='in_axes',
                                    value=ast.Tuple(
                                        elts=[ast.Constant(value=el) for el in in_axes],
                                        ctx=ast.Load()
                                    )
                                )]
                            ),
                ...
                for key, values in self.dynamic_variable_lift.items():
                    ...
                    if is_present:
                        ...
                        for ax in sorted(batched_axes):
                            vect_value = vect_loop[ax].id if isinstance(vect_loop[ax], ast.Name) else vect_loop[ax].attr
                            if original_shape and vect_value not in original_shape:
                                sub_elts.insert(ax, ast.Slice())
                            else:
                                if original_shape and vect_value in original_shape and len(original_shape) != len(sub_elts):
                                    sub_elts.insert(ax, ast.Slice())

                        if len(sub_elts) == 1:
                            node.func.value.slice = sub_elts[0]
                        else:
                            node.func.value.slice = ast.Tuple(elts=sub_elts, ctx=ast.Load())
                        
            self.reduction.dynamic_variable_lift = self.dynamic_variable_lift 
                
            self.reduction.func_name = self.func_name if hasattr(self, 'func_name') else None
            self.reduction.vectorization_axis = vectorization_context['vectorization_axis'] if vectorization_context else  {}
            self.reduction.loop_info = vectorization_context['loop_info'] if vectorization_context else {}
    ...
    def _is_vectorized_child_call(self, node: ast.Expr) -> bool:
        ...
        * At least one argument of the call corresponds to a loop variable
        ...
        try:
            ...
            if not (isinstance(rhs.func, ast.Attribute) and
    ...
    def _emit_scan_wrapped_call(
        ...
        try:
            ...
            for dim, lvar in loop_info.items():
                vect_axis = vectorization_context.get('vectorization_axis', {})
            ...
            carry_out_elts = [ast.Name(id=f'{v}_out', ctx=ast.Store()) for v in modified_vars]
            carry_ret_elts = [ast.Name(id=f'{v}_out', ctx=ast.Load())  for v in modified_vars]
    ...
    def visit_Expr(self, node: ast.Expr) -> ast.AST | List[ast.AST]:
        ...
        try:
            ...
            actual_args = [arg.id if isinstance(arg, ast.Name) else arg.attr for arg in value.args] 
            # NOTE:
            ...
            modified_vars = list(ret_info["var_modif_args"]) + list(ret_info["var_modif_attr"])
            # NOTE:
            ...
            var_modif_attr = list(ret_info["var_modif_attr"]) + [vars for vars in ret_info["var_modif_args"] if vars in attributes]
            ...
            new_assign = ast.Assign(
                targets=[ast.Tuple(elts=tuple_targets, ctx=ast.Store())],
                value=new_call
            )
            ...
            if vectorization_context and self._is_vectorized_child_call(node):
                ...
                vmap_stmts = self._emit_scan_wrapped_call(
                    new_assign, vectorization_context,
            ...
            prev_attr_used = set([self.to_arg(value) for value in self._first_reads(fn_node.body)]) 
            ...
            if ((self._mutated_attrs | self._var_modif["attr"]) & (prev_attr_used | set(var_modif_attr)) - {'self'}):
                pre_self_attributes = (self._mutated_attrs | self._var_modif["attr"]) & (prev_attr_used | set(var_modif_attr)) - {'self'} 
                ...
                tree_at_call = ast.Call(
                    ...
                    args=[
                        ...
                        ast.Name(id="self", ctx=ast.Load()),
                        ast.Tuple(
                            elts=[ast.Name(id=a, ctx=ast.Load()) for a in pre_self_attributes],
                            ctx=ast.Load(),
                        ),
                    ],
                    keywords=[],
            ...
            current_modified = set(self._modified_ret_stack[-1]) if self._modified_ret_stack else set()
            if self._scan_stack:
                ...
                current_modified = [vars for vars in current_modified if vars in attributes]
    ...
    def visit_Attribute(self, node: ast.Attribute) -> ast.AST:
        ...
        try:
            ...
            if isinstance(node.value, ast.Name) and node.value.id == "self":
                ...
                if attr in self._mutated_attrs and attr in self._var_modif['attr'] \
                    or (self._func_arg_stack[-1] and attr in self._func_arg_stack[-1]) \
                    or (self._modified_ret_stack and attr in self._modified_ret_stack[-1]):
    ...
    def analyze_function_statefulness(self, func_node: ast.FunctionDef) -> None:
        ...
        * The ``self`` reference.
        ...
        try:
            ...
            already_defined = set(self.index_variables) | set(getattr(self, 'outer_scope_vars', set()))
            for arg in func_node.args.args:
                already_defined.add(arg.arg)
            # Also mark any class attributes / self.x as defined
            already_defined.add('self')

            self._stateful_vars_stack.append(collect_reads_before_def(func_node.body, already_defined))
        except Exception as e:
