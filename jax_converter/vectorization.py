...
from jax_utils import get_name, Control, MaybeAddIndexTransformer, contains_name
...
class _Vectorization:
    ...
    def get_vectorized_arrays(
        ...
        try:
            ...
            for sub in ast.walk(node):
                if isinstance(sub, ast.Subscript):
                    indices = sub.slice.elts if isinstance(sub.slice, ast.Tuple) else [sub.slice]
    ...
    def get_declared_dims(self, node: ast.AST) -> Optional[List]:
        """
        ...
        try:
            ...
            if isinstance(node, ast.Name):
                ...
                elif hasattr(self, 'func_input_dim') and node.id in list(self.func_input_dim.keys()):
            ...
            if isinstance(node, ast.Attribute):
                if isinstance(node.value, ast.Name):
                    ...
                    elif hasattr(self, 'func_input_dim') and attr in list(self.func_input_dim.keys()):
            ...
            if isinstance(node, ast.Subscript):
                ...
                if isinstance(sl, ast.Tuple):
                    for dim, idx in zip(declared, sl.elts):
                        if isinstance(idx, (ast.Name, ast.Attribute, ast.BinOp, ast.Constant)):
                            continue
                        active.append(dim)
                else:
                    active = declared[1:]
    ...
    def _target_rank(self, slice_node: ast.AST) -> int:
        ...
        try:
            ...
            if self._control_stack:
                vec_context = self._control_stack[-1].to_dict().get('vectorization_axis')
            else:
                return 1

            if isinstance(slice_node, ast.Tuple):
                return sum(
                    isinstance(e, ast.Name) and e.id in vec_context or isinstance(e, ast.Slice)
    ...
    def _base_symbol(self, node: ast.AST) -> Optional[ast.AST]:
        """
    ...
    def is_class_attribute(self, arg: ast.AST) -> bool:
        ...
        try:
            ...
            if isinstance(arg, ast.BinOp):
                return self.is_class_attribute(arg.left) or self.is_class_attribute(arg.right)
            return False
        except Exception as e:
    ...
    def _is_arange_over_dim(self, node: ast.AST) -> bool:
        ...
        try:
            if isinstance(node, ast.BinOp):
                return self._is_arange_over_dim(node.left) or self._is_arange_over_dim(node.right)
        ...
        try:
            ...
            if isinstance(node, ast.Subscript):
                ...
                if isinstance(sl, ast.Tuple):
                    rank = 0
                    for l in sl.elts:
                        if isinstance(l, ast.Slice) and self.is_full_slice(l):
                            rank += 1
                        elif isinstance(l, ast.Constant) and l.value is None:
                            rank += 1
                        elif self._is_arange_over_dim(l):
                            rank += 1
                else:
            ...
            if isinstance(node, (ast.Name, ast.Attribute)):
                ...
                rank = len(self.get_declared_dims(node)) if self.get_declared_dims(node) else 0
            ...
            if isinstance(node, ast.Call):
                ...
                if func_name in {'sum', 'mean', 'prod'}:
                    ...
                    for kw in node.keywords:
                        if kw.arg == 'keepdims' and isinstance(kw.value, ast.Constant) and kw.value.value:
                            rank = input_rank
            ...
            if isinstance(node, ast.Compare):
                ...
                left_rank = left_ranks.get(node.left, 0)
                right_rank = max((comp_ranks.get(comp, 0) for comp in node.comparators), default=0)
                ...
                if left_rank == 0 and isinstance(node.left, ast.Name):
                    ...
                    if left_rank == 0 and (
                        ...
                        or (state := self.var_state.get(node.left.id)) and state[0] == 'stateful'
                    ):
                        left_rank = 1
                    ...
                    for comp in node.comparators:
                        if isinstance(comp, ast.Name):
                            ...
                            if comp_rank == 0 and (
                                ...
                                or (state := self.var_state.get(comp.id)) and state[0] == 'stateful'
                            ):
                                comp_rank = 1
        ...
        *target_rank*.
        ...
        try:
            ...
            transformer = MaybeAddIndexTransformer(
                ...
                local_defined_variables=self._local_defined_stack[-1] if self._local_defined_stack else [],
                dynamic_variable_lift=self.dynamic_variable_lift if self.dynamic_variable_lift else {},
                inferred_ranks=self._inferred_ranks,
                func_input_dim=self.func_input_dim if hasattr(self, 'func_input_dim') else None,
            )
    ...
    def _apply_broadcasting_slice(self, sub: ast.Subscript, assign_vect: Dict):
        ...
        try:
            if isinstance(sub.slice, tuple):
                new_slices = []
                for s in sub.slice:
                    if isinstance(s, ast.Name) and s.id in assign_vect['vectorization_axis']:
                ...
                if isinstance(sub.slice, ast.Name) and sub.slice.id in assign_vect['vectorization_axis']:
                    sub.slice = ast.Slice()
                    return ast.Slice()
        except Exception as e:
    ...
    def _rhs_is_pairwise(self, node: ast.AST) -> bool:
        ...
        try:
            ...
            for child in ast.walk(node):
                if isinstance(child, ast.Subscript) and getattr(child, '_pairwise', False):
                    found = True
                    break
            return found
        except Exception as e:
    ...
    def _make_arange(self, axis: List[str], loop_info: Dict) -> ast.Call:
        ...
        try:
            ...
            loop_var = next((key for key, val in loop_info.items() if val in axis), None)

            if loop_var is None:
                raise ValueError(f"No loop_info entry found for index '{axis}'")

            if loop_var in attributes:
                val = ast.Attribute(value=ast.Name(id='self', ctx=ast.Load()), attr=loop_var, ctx=ast.Load())
            else:
    ...
    def _expr_depends_on_axes(self, node: ast.AST, vectorization_axis: Dict) -> Tuple:
        ...
        try:
            ...
            if isinstance(node, ast.Name) and not node.id in vectorization_axis:
    ...
    def _analyze_local_assignment(
        ...
        try:
            ...
            lhs_uses_loop = self._subscript_uses_loop_vars(target_node, list(axes.keys()))
            rhs_uses_loop = self._subscript_uses_loop_vars(node.value, list(axes.keys()))
    ...
    def _build_vectorized_loop(self, loop_info: Dict, axes: Dict) -> List[ast.AST]:
        ...
        try:
            ...
            if loop_var in attributes:
                return [ast.Attribute(value=ast.Name(id='self', ctx=ast.Load()), attr=loop_var, ctx=ast.Load())]
    ...
    def _emit_convergence_break(
        ...
        try:
            ...
            update_stmts = [
                s for s in (node.orelse if break_in_body else node.body)
            ...
            if vectorization_context:
                ...
                self._control_stack.append(
                    Control(
                        ...
                        metadata={'current_mask_assign': active_mask_assign, 'current_mask_rank': mask_rank},
                    )
                )
                ...
                if isinstance(visited, list):
                    for v in visited:
                        ...
                        if isinstance(v, ast.Assign):
                            new_stmts.extend(self._mask_vector_assign(v, active_mask_name, assigned, used_after))
                        else:
                            new_stmts.append(v)
                elif isinstance(visited, (ast.Break, ast.Continue)):
                    pass
                elif isinstance(visited, ast.Assign):
                    new_stmts.extend(self._mask_vector_assign(visited, active_mask_name, assigned, used_after))
            ...
            converged_update = ast.Assign(
                ...
                value=ast.Call(
                    func=ast.Attribute(value=ast.Name(id='jnp', ctx=ast.Load()), attr='logical_or', ctx=ast.Load()),
                    args=[ast.Name(id=converged_name, ctx=ast.Load()), ast.Name(id=break_mask, ctx=ast.Load())],
                    keywords=[],
                ),
            )
    ...
    def _get_or_create_converged_var(self, vectorization_context: Optional[Dict]) -> str:
        """
        ...
        try:
            ...
            vect_dim = (
                ...
                if vectorization_context else 'kjpindex'
            )
            init_stmt = ast.Assign(
                targets=[ast.Name(id=name, ctx=ast.Store())],
                value=ast.Call(
                    func=ast.Attribute(value=ast.Name(id='jnp', ctx=ast.Load()), attr='zeros', ctx=ast.Load()),
                    args=[ast.Attribute(value=ast.Name(id='self', ctx=ast.Load()), attr=vect_dim, ctx=ast.Load())],
                    keywords=[ast.keyword(
                        arg='dtype',
                        value=ast.Attribute(value=ast.Name(id='jnp', ctx=ast.Load()), attr='bool_', ctx=ast.Load()),
                    )],
                ),
            )
