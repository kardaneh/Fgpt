...
from jax_utils import get_name, Control
...
class _LoopLowering:
    ...
    def visit_For(self, node: ast.For) -> ast.AST | List[ast.AST] | None:
        ...
        try:
            ...
            range_present = (isinstance(node.target, ast.Name) 
            ...
            if any(isinstance(node, ast.Name) and node.id in self._stateful_vars for node in (start, stop, step) if node is not None):
                self.symbol_table[loop_index] = 'int64'
                return self.handle_dynamic_loop(node, start, stop, step, loop_index)
            ...
            if cond_type == 'index_loop':
                ...
                self._push_scope()
                try:                
                    vectorization_axis = self._setup_vector_control(loop_index, get_name(stop), cond_type, node)
                    ...
                    for stmt in node.body:
                        ...
                        if isinstance(visited, list):
                            ...
                            if isinstance(stmt.targets[0], ast.Subscript):
                                ...
                                visited.value, _  = self.maybe_add_index(visited.value, target_rank, vectorization_context)
    ...
    def handle_scan(
        ...
        * :attr:`_modified_ret_stack`
        ...
        try:
            ...
            prev_scan_ = []
            if self._scan_stack: 
                prev_scan_ = list(self._scan_stack[-1]["mutated"] | self._scan_stack[-1]["introduced"])
            # This does region based aggregation
            start_idx = len(self._modified_ret_stack)
            self._modified_ret_stack.append([])

            inside_helper = (len(self._context_stack) > 0 and any(self.to_arg(name) in self._context_stack[-1]['helper_args'] for name in self._mutated_attrs))
                ...
                if name.startswith("self."):
                    ...
                    if attr in self._mutated_attrs:
                        if inside_helper:
                            ...
                            # again.
                            if (self._func_arg_stack and attr in [self.to_arg(n) for n in self._func_arg_stack[-1]]) or attr in self._var_modif['attr']:
                                inputs.append(attr)
                            else:
                                inputs.append(name)
                ...
                if isinstance(transformed, list):
                    for v in transformed:
                        if isinstance(v, ast.Assign):
                            if vectorization_context and vectorization_context['metadata'].get('current_mask_assign'):
                                mask_name = vectorization_context['metadata'].get('current_mask_assign')
                                helper_fn.body.extend(self._mask_vector_assign(v, mask_name.targets[0].id, assigned, used_after))
                            else:
                    ...
                    if vectorization_context and vectorization_context['metadata'].get('current_mask_assign'):
                        mask_name = vectorization_context['metadata'].get('current_mask_assign')
                        helper_fn.body.extend(self._mask_vector_assign(transformed, mask_name.targets[0].id, assigned, used_after))
                    else:
                        if isinstance(stmt, ast.Assign) and isinstance(stmt.targets[0], ast.Subscript):
                            target_rank = self._target_rank(stmt.targets[0].slice)
                            transformed.value, _  = self.maybe_add_index(transformed.value, target_rank, vectorization_context)
            ...
            if local_modified:
                ...
                diff = set(local_modified) - ((self._mutated_attrs) | self._var_modif['attr'])
    ...
    def handle_vmap(
        ...
        try:
            ...
            for stmt in node.body:
                ...
                if isinstance(visited, list):
                    ...
                    for i, last_node in enumerate(visited):
                        if isinstance(last_node, ast.Assign) and is_eqx_while_loop_call(last_node.value):
                            targets = last_node.targets
                            for t in targets:
                                if isinstance(t, ast.Tuple):
                                    ret_names.extend(
                                        elt.id for elt in t.elts if isinstance(elt, ast.Name)
                                    )
            ...
            fn_def = ast.FunctionDef(
                ...
                args=ast.arguments(
                    posonlyargs=[],
                    args=[ast.arg(arg=name, annotation=None) for name in self.while_transformer.ji_dependent_vars] if self.while_transformer.ji_dependent_vars else [],
                    kwonlyargs=[], kw_defaults=[], defaults=[]
                ),
            ...
            if len(ret_names) == 1:
                ...
                return_value = ast.Tuple(elts=[ast.Name(id=name, ctx=ast.Load()) for name in ret_names], ctx=ast.Load())
                targets = [ast.Tuple(
            ...
            assign_stmt = ast.Assign(
                ...
                value=ast.Call(
                    func= ast.Call(
                        func=ast.Attribute(value=ast.Name(id='jax', ctx=ast.Load()), attr='vmap', ctx=ast.Load()),
                        args=[ast.Name(id=fn_def.name, ctx=ast.Load())], keywords=[]
                    ),
                    args=[ast.Name(id=name, ctx=ast.Load()) for name in self.while_transformer.ji_dependent_vars],
                    keywords=[]
                )
            )
    ...
    def _setup_vector_control(
        ...
        try:
            ...
            vectorization_axis = self.compute_vectorization_axes(node, loop_index=loop_index)
            ...
            if not self._control_stack:
                self._control_stack.append(Control("loop", loop_info={stop:loop_index}, transform_type=cond_type, vectorization_axis=vectorization_axis))
            else:
    ...
    def get_initial_values_from_stack(self, inputs: List[str]) -> Dict[str, ast.AST | None]:
        """
                ...
                for stmt in body:
                    if isinstance(stmt, ast.Assign):
                        for target in stmt.targets:
                            ...
                            if var_name in initial_values and initial_values[var_name] is None:
    ...
    def visit_While(self, node: ast.While) -> List[ast.AST]:
        ...
        try:
            ...
            is_vector, vector_arrays = self.check_condition(node.test, vectorization_context)
            ...
            if not all(list(intial_values.values())):
                raise ValueError(f'Intial_values dict is empty')
            init_state = ast.Assign(targets=[ast.Name(id='init_state',ctx=ast.Store())], 
                                    value=ast.Tuple(elts=list(intial_values.values()), 
                                    ctx=ast.Load()))    
            ...
            if is_vector:
                ...
                cond_fn = ast.FunctionDef(
                    name='cond_fn', args=ast.arguments(
                        posonlyargs=[],
                    ...
                    body=[
                        ast.Assign(targets=[ast.Tuple(elts=[ast.Name(id=el, ctx=ast.Store()) for el in state_inputs])], 
            ...
            body_fn = ast.FunctionDef(
                name='body_fn', args=ast.arguments(
                    posonlyargs=[],
                    args=[ast.arg(arg='state')],
                    kwonlyargs=[],
                    kw_defaults=[],
                ...
                node.body + 
                [ast.Return(value=ast.Tuple(elts=[ast.Name(id=el, ctx=ast.Load()) for el in state_inputs],ctx=ast.Load()))], 
                decorator_list=[] 
            ...
            if self.mode in  ['fwd', 'jax']:
                kind = ast.keyword(
                        arg='kind',
                        value=ast.Constant(value='lax')
                    )
            else:
                kind = ast.keyword(
                        arg='kind',
                        value=ast.Constant(value='checkpointed')
                    )
            input_elts = []
            ...
            while_cond = ast.Assign(targets=[ast.Tuple(elts=input_elts,ctx=ast.Store())], 
                        value=ast.Call(func=ast.Attribute(
                            value=ast.Attribute(value=ast.Name(id='eqx', ctx=ast.Load()), attr='internal', ctx=ast.Load()),
                            attr='while_loop',
                            ctx=ast.Load()
                        ),
                        args=[ast.Name(id='cond_fn', ctx=ast.Load()), ast.Name(id='body_fn', ctx=ast.Load()),
    ...
    def simplify_range_args(
        ...
        * Any other step value is preserved.
        ...
        NotImplementedError
            ...
            expressions.
        """
        if not (self.is_static_expr(start) and self.is_static_expr(stop) and self.is_static_expr(step)):
