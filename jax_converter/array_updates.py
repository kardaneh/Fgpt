        ...
        if node.targets and isinstance(node.targets[0], ast.Subscript):
            ...
            if isinstance(sub_modified, ast.Subscript):
                ...
                if not vectorization_axis:
                    ...
                    axis = list(vectorization_axis.keys()) + ([loop_index] if loop_index else []) 
                    ...
                    if isinstance(sub.slice, ast.Slice):
                        ...
                        sub.slice.elts = [ast.Slice() if is_axis(node) else node for node in sub.slice.elts]
            ...
            if isinstance(sub.value, ast.Name):
                ...
                at_call = ast.Call(
                    func=ast.Attribute(
                        value=ast.Subscript(
                            value=ast.Attribute(value=ast.Name(id=arr_name, ctx=ast.Load()),
                                                attr='at', ctx=ast.Load()),
                            slice=slice_value,
                            ctx=ast.Load()
                        ),
                        attr=operator,
            ...
            if isinstance(sub.value, ast.Attribute) and isinstance(sub.value.value, ast.Name) and sub.value.value.id == "self":
                arr_name = sub.value.attr
                if sub.value.attr in self._mutated_attrs: 
                    at_call = ast.Call(
                        func=ast.Attribute(
                            value=ast.Subscript(
                                value=ast.Attribute(value=ast.Name(id=sub.value.attr, ctx=ast.Load()),
                                                    attr='at', ctx=ast.Load()),
                                slice=slice_value,
                                ctx=ast.Load()
                            ),
                    ...
                    at_call = ast.Call(func=ast.Attribute(
                        value=ast.Subscript(value=ast.Attribute(
                            value = ast.Attribute(
                ...
                new_assign = ast.Assign(targets=[ast.Name(id=sub.value.attr, ctx=ast.Store())], value=at_call)
                            ...
                            if is_self_dependent or is_control_flow_dependant: 
                                if var_name in self.var_state:
                                    if self.var_state[var_name][0] == "temporary":
                                        self.var_state[var_name] = ("stateful", self.var_state[var_name][1])
                                else:
                                ...
                                if origin_name not in self.dynamic_variable_lift:
                                    ...
                                    for dim in shape:
                                        if dim in attributes:
                                            ...
                                            vect_loop.append(ast.Name(id=dim, ctx=ast.Load()))

                                    # register lift
                                    dtype = self.infer_dtype(self.var_state[origin_name][1].value)
                                    ...
                                    if origin_name not in self.dynamic_variable_lift:
                                        ...
                                        if (self.func_name, origin_name) not in self._lifted_vars:
                                            self._lifted_vars.add(origin_name)
                                            func_args = {
                                                "func_name": self.func_name,
                                                "local_arr": {
                                                    "var": origin_name,
                                                    "update_value": {"dimensions": list(shape), "dtype": dtype, "type": "jnp.ndarray"}
                                                }
                                            }
        ...
        try:
            ...
            if isinstance(rhs, ast.BinOp): 
                ...
                if isinstance(left_inner, ast.Name):
                    ...
                    inner_name = (
                        left_inner.value.id if isinstance(left_inner.value, ast.Name)
        ...
        try:
            ...
            if attribute:
                if not attribute_args:
                    raise ValueError("If attribute=True, attribute_args must be provided")
    ...
    def visit_Subscript(self, node: ast.Subscript) -> ast.AST:
        ...
        * May record lifted array dimensions and vectorization information.
        ...
        if lift_info:
            ...
            for ax in sorted(batched_axes):
                ...
                if vect_value not in original_shape:
                    ...
                    if vect_value in original_shape and len(original_shape) != len(indices):
                        indices.insert(ax, ast.Slice())
            ...
            if (self.func_name, var_name) not in self._lifted_vars:
                ...
                for idx in sorted(batched_axes):
                    new_dim.insert(idx, 
                                   vect_loop[idx].id if isinstance(vect_loop[idx], ast.Name) else vect_loop[idx].attr)            
                self._lifted_vars.add((self.func_name, var_name))
                func_args = {
                    "func_name": self.func_name,
                    "local_arr": {
                        "var": var_name,
                        "update_value": {"dimensions": new_dim, 
            ...
            if deps:
                ...
                if any(
                    (
                        ...
                        and n.id in self.dynamic_variable_lift
                    )
                    or (
                        isinstance(n, ast.Name)
        ...
        for idx in indices:
            if needs_pairwise and is_axis(idx):
                ...
                if n_layers: # This is from the loop to mask for dynamic loops 
                    ...
                    lower = ast.BinOp(left=n_layers[0], op=ast.Add(), right=ast.Constant(value=offset)) if offset else n_layers[0]
                    upper = ast.BinOp(left=n_layers[1], op=ast.Add(), right=ast.Constant(value=offset)) if offset else n_layers[1]
        ...
        if not lift_info and all(self.is_full_slice(i) for i in new_indices):
            if isinstance(node.value, ast.Attribute) and getattr(node.value, 'attr', None) == 'at':
