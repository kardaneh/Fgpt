...
class _ConditionalLowering:
    ...
    def _rewrite_if_return(
        ...
        """
        try:
            true_body = [s for s in if_stmt.body \
                         if not isinstance(s, ast.Return)]
            false_body = [s for s in fn.body[idx + 1:] \
                          if not isinstance(s, ast.Return)]
    ...
    def _handle_lax_cond(
        ...
        try:
                    ...
                    if attr in self._mutated_attrs:
                        if inside_helper:
                            ...
                            if (
                                ...
                                and attr in [self.to_arg(n) for n in self._func_arg_stack[-1]]
            ...
            arg_stack = set()
            for name in inputs:
                if '.' in name and name.split('.', 1)[1] in self._mutated_attrs and inside_helper:
            ...
            if assigned and arg_set:
                outputs = set(assigned) & arg_set if set(assigned) & arg_set else set(assigned)

            true_name, false_name = self._fresh_names()

            def make_fn(name, body_stmts, inputs, outputs):
                arg_node = ast.arguments(
                    posonlyargs=[], args=[ast.arg(arg='arg')], kwonlyargs=[],
                    ...
                    if isinstance(visited, list):
                        for v in visited:
                            if isinstance(v, ast.Assign) and mask_name:
                                new_body.extend(
                                    self._mask_vector_assign(v, mask_name, assigned, used_after)
                                )
                        ...
                        new_body.extend(
                            self._mask_vector_assign(visited, mask_name, assigned, used_after)
                        )
                ...
                new_inputs = []
                if local_modified:
                    diff_inputs = set(local_modified) - set(self.to_arg(i) for i in inputs)
                    if self._scan_stack:
                        carry = set()
                        for frame in self._scan_stack:
                            carry.update(frame.get('introduced', set()) | frame.get('mutated', set()))
                ...
                input_args = [ast.Name(id=self.to_arg(n), ctx=ast.Store()) for n in inputs]
                ...
                if local_modified:
                    outputs = (outputs | set(local_modified)) if outputs else set(local_modified)
                ...
                fn = ast.FunctionDef(name=name, args=arg_node, body=body, decorator_list=[])
                return ast.fix_missing_locations(fn), outputs, new_inputs

            true_fn, outputs, new_inputs = make_fn(true_name, node.body, inputs, outputs)
            false_fn = None
            if node.orelse:
            ...
            tuple_in = (
                ast.Tuple(elts=[self.make_operand(n) for n in arg_stack], ctx=ast.Load())
                if inputs else ast.Constant(value=None)
            )
            ...
            if false_fn:
                ...
                self_vars = {v.split('.', 1)[1] for v in inputs if v.startswith('self.')}
                corrected_outputs = [
                    ('self.' + out) if out in self_vars else out for out in outputs
                ]
                false_case = ast.Lambda(
                    args=ast.arguments(
                        posonlyargs=[], args=[ast.arg(arg='_')], kwonlyargs=[],
                        kw_defaults=[], defaults=[],
                    ),
                    body=ast.Tuple(
                        elts=[ast.Name(id=o, ctx=ast.Load()) for o in corrected_outputs],
                        ctx=ast.Load(),
                    ),
                )

            cond_call = ast.Call(
                func=ast.Attribute(value=ast.Name(id='lax', ctx=ast.Load()), attr='cond', ctx=ast.Load()),
                args=[test, ast.Name(id=true_name, ctx=ast.Load()), false_case, tuple_in],
                keywords=[],
            )

            if outputs:
                assign = ast.Assign(
                    targets=[ast.Tuple(elts=[ast.Name(id=o, ctx=ast.Store()) for o in outputs], ctx=ast.Store())],
                    value=cond_call,
                )
            else:
                assign = ast.Assign(
    ...
    def visit_If(self, node: ast.If):
        ...
        try:
            ...
            node.orelse = [stmt for stmt in node.orelse if not self._is_logging_call(stmt)]
        ...
        try:
            ...
            for stmt_true, stmt_false in zip(assigns_true, assigns_false):
                ...
                if isinstance(target_true, ast.Name):
                    stmts_true_list = (
                        [self.visit(s) for s in stmt_true] if isinstance(stmt_true, list)
                        else [self.visit(stmt_true)]
                    )
                    stmts_false_list = (
                        [self.visit(s) for s in stmt_false] if isinstance(stmt_false, list)
                    ...
                    if self.var_deps.get(target_true.id):
                        ...
                        true_value = self.broadcast_scalar(true_value, deps_true, target_axes)
                        false_value = self.broadcast_scalar(false_value, deps_false, target_axes)

                    jnp_call = ast.Call(
                        func=ast.Attribute(value=ast.Name(id='jnp', ctx=ast.Load()), attr='where', ctx=ast.Load()),
                        args=[self._transform_if_test(node.test), true_value, false_value],
                        keywords=[],
                    )
                    ...
                    if not isinstance(stmt_true, ast.Assign) or not isinstance(stmt_false, ast.Assign):
                    ...
                    if isinstance(stmt_true.value.func, ast.Attribute) and isinstance(stmt_false.value.func, ast.Attribute):
                    ...
                    true_args, true_elts_list = self.maybe_add_index(true_args, target_rank, vectorization_context)
                    false_args, false_elts_list = self.maybe_add_index(false_args, target_rank, vectorization_context)
                    ...
                    selected_elts_list = true_elts_list if true_elts_list else false_elts_list
                    ...
                    jnp_call = ast.Call(
                        func=ast.Attribute(value=ast.Name(id='jnp', ctx=ast.Load()), attr='where', ctx=ast.Load()),
                        args=[mask_expr if mask_expr else mask_name, true_args, false_args],
                        keywords=[],
                    )
                    jnp_call = self.visit(ast.fix_missing_locations(jnp_call))

                    if same_operation:
                        value = ast.Call(func=stmt_true.value.func, args=[jnp_call], keywords=[])
                        ...
                        ops = {
                            ...
                            'divide': ast.Div, 'power': ast.Pow,
                        }

                        if operation_true in ops:
                            true_expr = ast.BinOp(left=old_val_name, op=ops[operation_true](), right=true_args)
                        else:
                            true_expr = true_args

                        if operation_false in ops:
                            false_expr = ast.BinOp(left=old_val_name, op=ops[operation_false](), right=false_args)
                        else:
                            false_expr = false_args

                        jnp_call = ast.Call(
                            func=ast.Attribute(value=ast.Name(id='jnp', ctx=ast.Load()), attr='where', ctx=ast.Load()),
                            args=[mask_expr if mask_expr else mask_name, true_expr, false_expr],
                            keywords=[],
                        )
                        jnp_call = self.visit(ast.fix_missing_locations(jnp_call))
    ...
    def _lower_vector_condition(
        ...
        try:
            ...
            ranks = self.subscript_ranks(node.test)
            mask_rank = ranks.get(node.test, 0)
            cond_copy = self._replace_subscript_with_name(ast.copy_location(node.test, node))
            mask_assign = ast.Assign(targets=[ast.Name(id=mask_name, ctx=ast.Store())], value=self.visit(cond_copy))
            mask_assign = self._boolean_mask(mask_assign)
            ...
            if vectorization_context:
                self._control_stack.append(
                    Control(
                        ...
                        vectorization_axis=vectorization_context['vectorization_axis'],
                        metadata={'current_mask_assign': mask_assign, 'current_mask_rank': mask_rank},
                    )
                )
                ...
                if isinstance(visited, list):
                    for v in visited:
                        if isinstance(v, ast.Assign):
                            new_stmts.extend(self._mask_vector_assign(v, mask_name, assigned, used_after))
                        elif isinstance(v, ast.Continue):
                            continue
                        else:
                            new_stmts.append(v)
                elif isinstance(visited, ast.Assign):
                    new_stmts.extend(self._mask_vector_assign(visited, mask_name, assigned, used_after))
                elif isinstance(visited, ast.Continue):
                    pass
                elif visited is not None:
            ...
            if node.orelse != []:
                ...
                for stmt in node.orelse:
                    ...
                    if isinstance(visited, list):
                        for v in visited:
                            if isinstance(v, ast.Assign):
                                new_stmts.extend(self._mask_vector_assign(v, else_mask_name, assigned, used_after))
                            elif isinstance(v, ast.Continue):
                                continue
                            else:
                                new_stmts.append(v)
                    elif isinstance(visited, ast.Assign):
                        new_stmts.extend(self._mask_vector_assign(visited, else_mask_name, assigned, used_after))
                    elif isinstance(visited, ast.Continue):
                        pass
                    elif visited is not None:
                        new_stmts.append(visited)
