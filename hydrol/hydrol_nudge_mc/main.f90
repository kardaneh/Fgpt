...
&+ 1) * (trois * mc_aux(ji, jsl, jst) + mc_aux(ji, jsl + 1, jst)) / huit
  ...
  SUBROUTINE read_dummy(jst, mc_loc)
    REAL(KIND = r_std), DIMENSION(kjpindex, nslm, nstm) :: mc_loc
    INTEGER(KIND = i_std) :: jst
    OPEN(UNIT = 1363, FILE = '/net/nfs/ssd1/kardaneh/Fgpt/benchmark/hydrol_nudge_mc/dummy.bin', FORM = 'unformatted', STATUS = &
&'old')
    WRITE(*, *) '--- inside the read dummy routine for hydrol_nudge_mc ---'
    READ(1363, IOSTAT = ier) jst
    IF (ier /= 0) THEN
      WRITE(*, *) 'Error reading from file for jst. ', ' IOSTAT : ', ier
    END IF
    READ(1363, IOSTAT = ier) mc_loc
    IF (ier /= 0) THEN
      WRITE(*, *) 'Error reading from file for mc_loc. ', ' IOSTAT : ', ier
    END IF
    CLOSE(UNIT = 1363)
