...
PROGRAM main
  ...
  CALL read_dummy(ins, mcr, mcs, njsc)
...
&jsl, ji))
  ...
  SUBROUTINE read_dummy(ins, mcr, mcs, njsc)
    INTEGER(KIND = i_std), DIMENSION(kjpindex) :: njsc
    REAL(KIND = r_std), DIMENSION(kjpindex) :: mcs
    REAL(KIND = r_std), DIMENSION(kjpindex) :: mcr
    INTEGER(KIND = i_std) :: ins
    OPEN(UNIT = 1363, FILE = '/net/nfs/ssd1/kardaneh/Fgpt/benchmark/hydrol_soil_coef/dummy.bin', FORM = 'unformatted', STATUS = &
&'old')
    WRITE(*, *) '--- inside the read dummy routine for hydrol_soil_coef ---'
