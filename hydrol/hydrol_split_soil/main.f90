...
PROGRAM main
  ...
  CALL read_dummy(e_frac, evap_bare_lim, evap_bare_lim_ns, F_absorption, humrel, soiltile, tot_bare_soil, transpir, us, veget_max, &
&vevapnu)
  CALL SYSTEM_CLOCK(ic0, icr, ic)
  start_time = ic0 * 1.0 / icr
  CALL hydrol_split_soil(kjpindex, veget_max, soiltile, vevapnu, transpir, humrel, evap_bare_lim, evap_bare_lim_ns, tot_bare_soil, &
&us, e_frac, F_absorption)
  CALL SYSTEM_CLOCK(ic0, icr, ic)
...
&vevapnu)
  ...
  !$ACC ENTER DATA COPYIN(veget_max, soiltile, vevapnu, transpir, humrel, evap_bare_lim, evap_bare_lim_ns, tot_bare_soil, us,  &
!$ACC& e_frac, F_absorption)
  CALL SYSTEM_CLOCK(ic0, icr, ic)
  start_time = ic0 * 1.0 / icr
  !$ACC PARALLEL LOOP INDEPENDENT REDUCTION(+:error_flag_hydrol_split_soil_1, error_flag_hydrol_split_soil_2,  &
!$ACC& error_flag_hydrol_split_soil_3, error_flag_hydrol_split_soil_4, error_flag_hydrol_split_soil_5,  &
!$ACC& error_flag_hydrol_split_soil_6, error_flag_hydrol_split_soil_7, error_flag_hydrol_split_soil_8,  &
!$ACC& error_flag_hydrol_split_soil_9, error_flag_hydrol_split_soil_10, error_flag_hydrol_split_soil_11,  &
!$ACC& error_flag_hydrol_split_soil_12)
  DO ji = 1, kjpindex
...
&tot_bare_soil, us, e_frac, F_absorption)
  ...
  stop_time = ic0 * 1.0 / icr
  WRITE(*, *) "Execution time : ", stop_time - start_time
  !$ACC UPDATE SELF(ae_ns, precisol_ns, rootsink, tr_ns)
  !$ACC EXIT DATA DELETE(veget_max, soiltile, vevapnu, transpir, humrel, evap_bare_lim, evap_bare_lim_ns, tot_bare_soil, us,  &
!$ACC& e_frac, F_absorption)
...
&'ROOTSINK SPLIT FALSE')
  ...
  SUBROUTINE read_dummy(e_frac, evap_bare_lim, evap_bare_lim_ns, F_absorption, humrel, soiltile, tot_bare_soil, transpir, us, &
&veget_max, vevapnu)
    REAL(KIND = r_std), DIMENSION(kjpindex) :: vevapnu
    REAL(KIND = r_std), DIMENSION(kjpindex, nvm) :: veget_max
    REAL(KIND = r_std), DIMENSION(kjpindex, nvm, nstm, nslm) :: us
    ...
    REAL(KIND = r_std), DIMENSION(kjpindex) :: evap_bare_lim
    REAL(KIND = r_std), DIMENSION(kjpindex, nvm, nslm, nstm) :: e_frac
    OPEN(UNIT = 1363, FILE = '/net/nfs/ssd1/kardaneh/Fgpt/benchmark/hydrol_split_soil/dummy.bin', FORM = 'unformatted', STATUS = &
&'old')
    WRITE(*, *) '--- inside the read dummy routine for hydrol_split_soil ---'
