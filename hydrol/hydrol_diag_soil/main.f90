...
PROGRAM main
  ...
  CALL read_dummy(avan, evapot, frac_snow_nobio, irrigation, ks, mcfc, mcr, mcs, mcw, njsc, nvan, precip_rain, reinfiltration, &
&returnflow, soiltile, tot_melt, totfrac_nobio, us, veget_max, vevapnu)
...
&drysoil_frac, tot_melt, us, precip_rain, totfrac_nobio, frac_snow_nobio)
  ...
  CALL read_dummy(avan, evapot, frac_snow_nobio, irrigation, ks, mcfc, mcr, mcs, mcw, njsc, nvan, precip_rain, reinfiltration, &
&returnflow, soiltile, tot_melt, totfrac_nobio, us, veget_max, vevapnu)
  !$ACC ENTER DATA COPYIN(ks, nvan, avan, mcr, mcs, mcfc, mcw, veget_max, soiltile, njsc, runoff, drainage, evapot, vevapnu,  &
!$ACC& returnflow, reinfiltration, irrigation, shumdiag, shumdiag_perma, k_litt, litterhumdiag, humrel, vegstress, drysoil_frac,  &
!$ACC& tot_melt, us, precip_rain, totfrac_nobio, frac_snow_nobio)
  CALL SYSTEM_CLOCK(ic0, icr, ic)
  start_time = ic0 * 1.0 / icr
...
&drysoil_frac, tot_melt, us, precip_rain, totfrac_nobio, frac_snow_nobio)
  ...
  stop_time = ic0 * 1.0 / icr
  WRITE(*, *) "Execution time : ", stop_time - start_time
  !$ACC UPDATE SELF(mc, tmc_litt_dry_mea, tmc_litt_mea, tmc, dr_ns, ae_ns, ru_ns, humrelv, soilmoist_s, soilmoist_liquid,  &
!$ACC& tmc_litt_wet_mea, profil_froz_hydro, soilmoist, humtot, drainage, drysoil_frac, humrel, k_litt, litterhumdiag, runoff,  &
!$ACC& shumdiag, shumdiag_perma, us, vegstress, vevapnu)
  !$ACC EXIT DATA DELETE(ks, nvan, avan, mcr, mcs, mcfc, mcw, veget_max, soiltile, njsc, runoff, drainage, evapot, vevapnu,  &
!$ACC& returnflow, reinfiltration, irrigation, shumdiag, shumdiag_perma, k_litt, litterhumdiag, humrel, vegstress, drysoil_frac,  &
!$ACC& tot_melt, us, precip_rain, totfrac_nobio, frac_snow_nobio)
...
&- mcw(ji)))
  ...
  SUBROUTINE read_dummy(avan, evapot, frac_snow_nobio, irrigation, ks, mcfc, mcr, mcs, mcw, njsc, nvan, precip_rain, &
&reinfiltration, returnflow, soiltile, tot_melt, totfrac_nobio, us, veget_max, vevapnu)
    ...
    REAL(KIND = r_std), DIMENSION(kjpindex) :: evapot
    REAL(KIND = r_std), DIMENSION(kjpindex) :: avan
    OPEN(UNIT = 1363, FILE = '/net/nfs/ssd1/kardaneh/Fgpt/benchmark/hydrol_diag_soil/dummy.bin', FORM = 'unformatted', STATUS = &
&'old')
    WRITE(*, *) '--- inside the read dummy routine for hydrol_diag_soil ---'
