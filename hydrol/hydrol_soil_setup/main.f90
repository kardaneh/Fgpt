...
PROGRAM main
  ...
  stop_time = ic0 * 1.0 / icr
  WRITE(*, *) "Execution time : ", stop_time - start_time
  !$ACC UPDATE SELF(gp, fp, e, g1, ep, f)
...
&free_drain_coef(ji, ins)))
  ...
  SUBROUTINE read_dummy(ins)
    INTEGER(KIND = i_std) :: ins
    OPEN(UNIT = 1363, FILE = '/net/nfs/ssd1/kardaneh/Fgpt/benchmark/hydrol_soil_setup/dummy.bin', FORM = 'unformatted', STATUS = &
&'old')
    WRITE(*, *) '--- inside the read dummy routine for hydrol_soil_setup ---'
