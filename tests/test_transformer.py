...
b = 2
        ...
        self.transformer.dependant_variables = {"c": ["a", "b"]}

        c_assign = ast.parse("c = a + b").body[0]

        updated_body = self.transformer.init_dependant_variables(read_ast, [c_assign])
...
for j, k in [(1,2)]:
    ...
    def test_prepare_read_code_global(self):
        ...
        result = self.transformer.prepare_read_code_for_global_template(
            assign_nodes, subroutine_key="test_sub"
        )
...
"""
        ...
        cls_info, imports, instances = self.transformer.create_cls_info(
            out_module=tree, subroutine_key="test_sub"
        )
        ...
        cls_info, _, instances = self.transformer.create_cls_info(
            out_module=tree, subroutine_key="test_sub", self_mode=True
        )
    ...
    def test_add_instance(self):
        ...
        assert isinstance(func.body[1], ast.Expr) and isinstance(
            func.body[1].value.func, ast.Attribute
        )  # call type of obj.declaration
        ...
        assert (
            len(func.body) == 1
        )  # If the method in question is not present inside the cls_info not the instance is added
...
def foo(a, b): pass
    ...
    def test_convert_specification(self):
        ...
        self.transformer.isolator.processor.remove_intent_and_save = MagicMock(
            return_value=[decl]
        )
        ...
        assert isinstance(result[0].value, ast.Call) and isinstance(
            result[0].value.func, ast.Attribute
        )
        ...
        assert (
            isinstance(result[0].value.args[0], ast.Tuple)
            and len(result[0].value.args[0].elts) == 2
        )
        assert len(result[0].value.keywords) == 1 and isinstance(
            result[0].value.keywords[0], ast.keyword
        )
        assert (
            isinstance(result[0].value.keywords[0].value, ast.Attribute)
            and result[0].value.keywords[0].value.attr == "float64"
        )
...
x: int = 5
    ...
    def test_search_dependant_variables(self):
        ...
        # Exception handling
        self.transformer._build_symbol_table = MagicMock(
    ...
    def test_separate_scalar_with_input(self, monkeypatch):
        ...
        self.transformer.separate_scalar(subroutine_key="test", dec_stmts=decls)
        ...
        self.transformer.extractor.dec_global = {"test": dec_global}
        ...
        tree = Processor(logger=Logger()).parse_fortran_statement(code)
        decls = list(walk(tree, F23.Type_Declaration_Stmt))
        self.transformer.extractor.var_dummy = {"test": decls}
    ...
    def test_insert_at(self):
        ...
        module = ast.Module(
            body=[ast.Import(names=[ast.alias(name="sys")])], type_ignores=[]
        )
    ...
    def test_insert_all_assign_nodes(self):
        ...
        assign_nodes = [self.make_assign("a", "b + 1"), self.make_assign("b", "1")]
