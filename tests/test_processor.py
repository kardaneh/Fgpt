...
def test_env(request):
    # use a real absolute temp directory for everything
    test_dir = tempfile.mkdtemp()
    benchmark_dir = os.path.join(test_dir, "benchmark")  # ← absolute, inside test_dir
    ...
    benchmark_subdir = os.path.join(benchmark_dir, "test_sub")
    ...
    # teardown — restore cwd first, then clean up
...
class TestProcessor:
    ...
    def test_find_enclosing_parent(self):
        ...
        assert isinstance(result, F23.Assignment_Stmt), (
            "Result should be Assignment_Stmt"
        )
        ...
        result = self.processor.find_enclosing_parent(
            name_node, F23.Subroutine_Subprogram
        )
        ...
        assert result is not None, (
            "Should return the node itself when it matches target type"
        )
        ...
        subroutine = self.processor.parse_fortran_string(
            "subroutine test()\n  zwholdmax(ji, :) = snow3lhold_1d(snowrho(ji, :), snowdz(ji, :))\nend subroutine"
        )
        ...
        assert isinstance(result, F23.Subroutine_Subprogram), (
            "Result should be Subroutine_Subprogram"
        )
    ...
    def test_add_entity_to_declaration(self):
        ...
        decl = "real :: a, b"
        parsed_decl = self.processor.parse_fortran_statement(decl)
        modified = self.processor.add_entity_to_declaration(
            parsed_decl.children[0], ["a"]
        )
        assert "a_copy" in modified.tostr()

        # Test with multiple variables
        modified = self.processor.add_entity_to_declaration(
            parsed_decl.children[0], ["a", "b"]
        )
    ...
    def test_add_entity_to_allocation(self):
        ...
        alloc = "allocate(a(10))"
        parsed_alloc = self.processor.parse_fortran_statement(alloc)
        modified = self.processor.add_entity_to_allocation(
            parsed_alloc.children[0], ["a"], openacc=True
        )
    ...
    def test_combine_allocate_declaration(self):
        ...
        combined = self.processor.combine_allocate_declaration(
            [parsed_decl.children[0], parsed_alloc.children[0]]
        )
    ...
    def test_check_point(self):
        ...
        checkpoint = self.processor.check_point(
            "flags", "flags_copy", ["LOGICAL", "DIMENSION"]
        )
        assert isinstance(checkpoint.children[0], F23.If_Construct)
    ...
    def test_process_queue(self):
        ...
        declarations = [
            ...
            self.processor.parse_fortran_statement(
                "integer, parameter :: a = 10"
            ).children[0],
            self.processor.parse_fortran_statement(
                "real, allocatable :: d(:)"
            ).children[0],
            self.processor.parse_fortran_statement("integer, parameter :: c").children[
                0
            ],
        ]
        ...
        assert (
            F23.Type_Declaration_Stmt("integer, parameter :: a = 10").tostr()
            == processed[0].tostr()
        )
        # Check that allocatable comes last
        assert (
            F23.Type_Declaration_Stmt("real, allocatable :: d(:)").tostr()
            == processed[-1].tostr()
        )
