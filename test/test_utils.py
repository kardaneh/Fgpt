...
from utils import AdjustIndices, ReplaceGlobals
...
def assert_ast_equal(a, b):
    assert ast.dump(a, include_attributes=False) == \
...
class TestAdjustIndices:
    ...
    def test_adjust_index(self):
        ...
        # Constant
        result = self.transformer._adjust_index(
    ...
    def test_apply_offset_conv_var(self):
        ...
        node = ast.Name(id="i", ctx=ast.Load())
        result = self.transformer._apply_offset_if_convvar(
            node,
            offset=-4
        )
        assert isinstance(result, ast.BinOp)
...
class TestReplaceGlobals:
    ...
    def test_get_attr_node(self):
        ...
        # Unknown node 
        assert self.replacer.get_attr_node(
            "unknown"
        ) is None
