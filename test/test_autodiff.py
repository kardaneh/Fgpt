...
def test_env(request):
    autodiff = AutoDiff(
        ...
        benchmark_dir=os.path.join('.', 'examples'),
        logger=Logger(),
...
class TestAutoDiff:
    ...
    def test_add_jax_imports(self, logger):
        ...
        logger.success('TEST passed for adding jax imports')
    ...
    def test_index_functions(self):
        ...
        code = 'def foo(): pass\ndef bar(): pass'
        functions = [n for n in ast.walk(_parse(code)) if isinstance(n, ast.FunctionDef)]
    ...
    def test_infer_subscript_shape(self):
        ...
        result = self.autodiff._infer_subscript_shape('not_a_list', ast.Constant(value=0))
        assert result is None
