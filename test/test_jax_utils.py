...
from jax_utils import ReductionHandler, VectorizationAnalyzer, MaybeAddIndexTransformer, WhileVectorToScalar
...
class TestVectorizationAnalyzer:
    ...
    def test_collect_loop_vars_from_for(self):
        node = _stmt(
            "for i in range(0, self.kjpindex):\n"
            "    pass"
        )
        assert self.analyzer._collect_loop_vars_from_for(node) == {"i"}

        node = _stmt(
            "for i in range(0, kjpindex):\n"
            "    pass"
        )
        assert self.analyzer._collect_loop_vars_from_for(node) == {"i"}
        ...
        # Tuple targets 
        node = _stmt(
            "for i, j in range(0, self.kjpindex):\n"
            "    pass"
        )
...
class TestMaybeAddIndexTransformer:
    ...
    def test_is_full_slice(self):
        ...
        assert not self.indexer.is_full_slice(
            ast.Slice(lower=ast.Constant(0))
        )
...
while cond:
    ...
    def test_visit_assign(self):
        ...
        self.whiletransformer.var_to_replace = {
            "x": "x_scalar"
        }
        ...
        result = self.whiletransformer.visit(node)
        assert ast.unparse(ast.fix_missing_locations(result)) == "x_scalar = x_scalar + y"
