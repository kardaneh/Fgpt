...
class TestF2NP:
    ...
    def test_handle_do_stmt(self):
        ...
        # start = 0 (1-1), end = 10
        assert isinstance(result.iter.args[0], ast.Constant) and result.iter.args[0].value == 0
        assert isinstance(result.iter.args[1], ast.Constant) and result.iter.args[1].value == 10
        assert isinstance(result.iter.args[2], ast.Constant) and result.iter.args[2].value == 1
        ...
        assert isinstance(result.iter.args[2], ast.Constant) and result.iter.args[2].value == 2
        ...
        if isinstance(stride_arg, ast.UnaryOp) and isinstance(stride_arg.op, ast.USub):
            assert isinstance(stride_arg.operand, ast.Constant) and stride_arg.operand.value == 1
        else:
    ...
    def test_handle_print_stmt(self):
        ...
        assert any(isinstance(v, ast.FormattedValue) and v.value.id == "var" for v in call.args[0].values)
        ...
        assert any(isinstance(v, ast.Constant) and v.value == 'Value:' for v in joined_values)
        assert any(isinstance(v, ast.FormattedValue) and v.value.id == "var" for v in joined_values)
    ...
    def test_handle_expr(self):
        ...
        assert isinstance(result, ast.BinOp)
        assert isinstance(result.left, ast.Constant) and isinstance(result.right, ast.Constant)
    ...
    def test_recursive_ast(self):
        ...
        assert len(result[0].body) == 1 # Here the real :: a,b, result are not transformed since they are function args neither implicit none
        ...
        assert [arg.arg for arg in func.args.args] == ["arr", "n", "threshold", "result"]
        # Structural checks
