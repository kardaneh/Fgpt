...
class Modifier:
    """
    Fortran code transformation engine for optimization and GPU adaptation.

    The Modifier class is responsible for rewriting and transforming Fortran
    source code to support modern execution environments such as GPUs and
    vectorized architectures. It performs structural and semantic code
    modifications on AST-level representations, enabling optimization and
    portability of legacy scientific Fortran code.

    Key responsibilities include:
    - Rewriting unsupported or non-portable Fortran constructs.
    - Transforming array operations and implicit shapes.
    - Managing vector loop insertion and loop restructuring.
    - Modifying conditional structures (IF / ELSE IF / ELSE).
    - Generating and adapting subroutine call patterns.
    - Supporting GPU-oriented transformations and memory movement patterns.

    Parameters
    ----------
    loop_vect_value : any
        Representation of a detected vector loop used for initialization.
    all_array_info : dict
        Metadata describing arrays detected in the codebase.
    loop_dict : dict
        Mapping of loop constructs and their extracted metadata.
    var_declared : set
        Set of variables declared in the current scope.
    imp_shape : list
        List of implicitly defined array shapes.
    io_subroutines : list
        List of I/O or external subroutines allowed during transformation.
    var_local : dict
        Mapping of local variables within subroutines.
    within_calls : dict, optional
        Call hierarchy information for nested subroutine analysis.
    child_error_flag : dict, optional
        Flags used to propagate transformation errors across AST nodes.

    Attributes
    ----------
    all_array_info : dict
        Global registry of array metadata extracted from the AST.
    loop_dict : dict
        Dictionary of detected loops and their structural metadata.
    var_declared : set
        Set of declared variables in the current subroutine/module.
    imp_shape : list
        Shapes of implicitly declared arrays.
    var_local : dict
        Mapping of local variables per scope.
    within_calls : dict or None
        Call graph information for nested transformations.
    child_error_flag : dict or None
        Error propagation flags for nested AST traversal.

    Notes
    -----
    This class operates on an intermediate AST representation and assumes
    prior parsing by the Processor and extraction by Extractor components.
    It is designed to be stateful and should not be reused across unrelated
    transformation pipelines without reinitialization.
    ...
    def __init__(
        ...
        self.vector_loop = (
            F23.Nonlabel_Do_Stmt(loop_vect_value) if loop_vect_value else None
        )
        self.dummy_add = (
            self.vector_loop.tostr().split("=")[0].split()[-1]
            if loop_vect_value
            else None
        )
    ...
    def contains_unsupported_function(
        self, assignment_stmt: F23.Assignment_Stmt
    ) -> bool:
        """
        ...
        Parameters
        ----------
        assignment_stmt: F23.Assignment_Stmt
            The Fortran assignment statement to check.

        Returns
        -------
        bool
            True if the statement contains any unsupported function, False otherwise.
        """
    ...
    def traverse_expression(self, node) -> None:
        """
        ...
        ----------
        node:
            The node to be traversed, which can be of various types
        ...
        try:
            if isinstance(node, F23.Part_Ref):
                ...
                name_str = node.tostr()
                if name_str in self.all_array_info.keys() and name_str not in self.seen:
                    self.arrays_queue.append(node)
    ...
    def add_dos(self, assignment_stmt: F23.Assignment_Stmt) -> None:
        """
        ...
        ----------
        assignment_stmt : F23.Assignment_Stmt
            The assignment statement to process.
        """
        try:
            ...
            array_names_all = [
                name
                for name in walk(assignment_stmt, F23.Name)
            ...
            while self.dims:
                ...
                if (
                    F23.Nonlabel_Do_Stmt(self.do_stmts[dim]).tostr()
                    != self.vector_loop.tostr()
                ):
                    self.code += self.do_stmts[dim] + "\n"
        except Exception as e:
            raise Exception(f"Error in add_dos: {e}")

    def extract_intrinsic_names(self, stmt) -> dict:
        """
        ...
        try:
            ...
            for intrinsic in walk(stmt, F23.Intrinsic_Name):
                ...
                assert len(intrinsic_parent.children) == 2, (
                    "Intrinsic parent must have exactly two children."
                )
                assert intrinsic_parent.children[0] == intrinsic, (
                    "First child must be the intrinsic function itself."
                )
                intrinsic_args = intrinsic.parent.children[1]
                assert isinstance(intrinsic_args, F23.Actual_Arg_Spec_List), (
                    "Second child must be an Actual_Arg_Spec_List."
                )
                args = intrinsic_args.children
                if len(args) == 1:
                    print(
                        f"Reduction operation detected for {args[0].tostr()} without an explicit 'DIM'. "
                ...
                if len(args) == 2:
                    if isinstance(args[1], F23.Actual_Arg_Spec):
                        ...
                        assert isinstance(dim_key, F23.Name), (
                            "First item in Actual_Arg_Spec must be a Name."
                        )
                        if dim_key.tostr().lower() == "dim":
                            assert isinstance(
                                dim_value_node, F23.Int_Literal_Constant
                            ), (
                                "Second item in Actual_Arg_Spec must be an Int_Literal_Constant."
                            )
                        ...
                        raise ValueError(
                            "Unexpected structure for intrinsic arguments."
                        )
                ...
                if dim_value:
                    dict_list.append(
                        {"dim": F23.Actual_Arg_Spec(f"DIM={dim_value}").tostr()}
                    )
    ...
    def process_section_subscript_list(self, node, reduction_dim) -> str | None:
        """
        ...
        ----------
        node
            The node containing the section subscript list.
        reduction_dim
            The dimension number for reduction.
    ...
    def modify_colon_array(self, node, in_intrinsic_parent=None, reduction_dim=None):
        ...
        node
            The node containing the section subscript list.
        in_intrinsic_parent
            Optional parent intrinsic for dimensional changes.
        reduction_dim
            Dimension to consider for reduction or modification.
                        ...
                        if dim.tostr() == ":":
                            ...
                            if in_intrinsic_parent is not None:
                                if reduction_dim == "ALL":
                                    ...
                                    shape[idim] = dim.tostr()
                                    print(
                                        f"The DIM value in {in_intrinsic_parent} must be changed to {colon_dim}."
                                    )
                                    new_arg_spec = F23.Actual_Arg_Spec(
                                        f"DIM = {colon_dim}"
                                    )
                                    continue
                            ...
                            assert (
                                lb == self.dos[index[0]][0]
                                and ub == self.dos[index[0]][1]
                            ), (
    ...
    def modify_colon_array_vec(
        self, node, in_intrinsic_parent=None, reduction_dim=None
    ):
        """
    ...
    def remove_vec_for_locals_in_assigns(self, node):
        ...
        ----------
        node : F23.Node
            The node representing an array.

        Returns
        -------
        F23.Node
            A modified node with the appropriate dimensions.
            ...
            for child in node.children:
                if isinstance(child, F23.Name):
                    ...
                    if array_name in self.var_local:
                        print(
                            f"Array '{array_name}' is a local array and the vector dim will be removed!"
                        )
                    else:
                        print(
                            f"Array '{array_name}' is a global/dummy array and the vector dim will be kept!"
                        )
                        return node
                    assert array_name in self.all_array_info, (
                ...
                if isinstance(child, F23.Section_Subscript_List):
                    ...
                    mod_node = (
                        F23.Part_Ref(f"{array_name}({dimensions})")
                        if dimensions
    ...
    def traverse_declaration_stmt(self, node):
        ...
        ----------
        node : F23.Node
            The node representing a declaration statement.

        Returns
        -------
        F23.Type_Declaration_Stmt
            A new type declaration statement constructed from the node.

        Raises
        ------
        Exception
            If any error occurs during the traversal or construction.
        """
        ...
        def inner_traverse(node):
            ...
            if isinstance(node, F23.Entity_Decl_List):
                any_shape_list = any(
                    isinstance(child, F23.Explicit_Shape_Spec_List)
        ...
        dimension_part = (
            f",DIMENSION({explicit_shape_spec_list})"
    ...
    def modify_array_without_notation(self, node):
        ...
        ----------
        node
            The node containing the array name.
            ...
            while self.arrays_queue:
                ...
                if ":" in node_str and isinstance(node, F23.Part_Ref):
                    ...
                    node_in_intrinsic = any(
                        node_str in key for key in intrinsic_name.keys()
                    )
                    ...
                    if node_in_intrinsic:
                        for intrinsic_parent in intrinsic_name.keys():
                            reduction_dim = "ALL"
                            print(
                                f"node {node_str} found within intrinsic parent {intrinsic_parent}"
                            )
                            if "dim" in intrinsic_name[intrinsic_parent]:
                                ...
                                print(
                                    f"{reduction_dim}th of :s in {node_str} must stay as < : >."
                                )
                                reduction_dim = self.process_section_subscript_list(
                                    node, reduction_dim
                                )
                                print(
                                    f"dimension {reduction_dim} of {node_str} must stay as < : >."
                                )

                            mod_node, new_arg_spec = self.modify_colon_array(
                                node, intrinsic_parent, reduction_dim
                            )
                            mod_intrinsic_parent = re.sub(
                                re.escape(node_str), mod_node.tostr(), intrinsic_parent
                            )
                            if new_arg_spec is not None:
                                mod_intrinsic_parent = re.sub(
                                    re.escape(old_arg_spec),
                                    new_arg_spec.tostr(),
                                    mod_intrinsic_parent,
                                    flags=re.IGNORECASE,
                                )
                            input_stmt = re.sub(
                                re.escape(intrinsic_parent),
                                mod_intrinsic_parent,
                                input_stmt,
                            )
                            node = mod_node
                        ...
                        input_stmt = re.sub(
                            re.escape(node_str), mod_node.tostr(), input_stmt
                        )
                        node = mod_node
    ...
    def replace_where(self, block) -> None:
        """
        ...
        ----------
        block
            The block containing WHERE constructs to be replaced.
        """
        try:
            if hasattr(block, "content"):
                ...
                while idc < len(block.content):
                    ...
                    if isinstance(child, F23.Where_Construct_Stmt):
                        ...
                        modified_stmt = self.modify_statement_where(child.children[0])
                        else_if_stmt = F23.Else_If_Stmt(
                            f"ELSEIF ({modified_stmt}) THEN"
                        )
                        ...
                        for key in self.dos.keys():
                            ...
                            if (
                                F23.Nonlabel_Do_Stmt(do_stmt).tostr()
                                != self.vector_loop.tostr()
                            ):
                                self.code += "ENDDO" + "\n"
    ...
    def modify_specification_part(self, block):
        ...
        ----------
        block
            The block containing entity declarations to be modified.
        ...
        try:
                ...
                while idc < len(block.content):
                    ...
                    if len(walk(declaration_stmt, F23.Entity_Decl)) > 1:
                        separate_declarations = (
                            Processor().separate_entity_declarations(declaration_stmt)
                        )
                ...
                if self.error_flag.keys():
                    for key in self.error_flag.keys():
                        block.content.insert(
                            first_idx + 1, self.error_flag[key]["error_flag_decl"]
                        )
            ...
            raise Exception(
                f"Error, an error occurred while modifying the specification part: {e}"
            )

    def remove_vec_for_locals_in_specification(self, block: F23.Block) -> F23.Block:
        """
        ...
        ----------
        block : F23.Block
            The block of statements to be processed.

        Returns
        -------
        F23.Block
            The updated block with modified type declaration statements.
        ...
        try:
            if hasattr(block, "content"):
                ...
                while idc < len(block.content):
                    ...
                    if not isinstance(declaration_stmt, F23.Type_Declaration_Stmt):
                        ...
                        if entity_decl not in self.var_local or not walk(
                            declaration_stmt, F23.Explicit_Shape_Spec_List
                        ):
                            idc += 1
                            continue
                        block.content[idc] = self.traverse_declaration_stmt(
                            declaration_stmt
                        )
                    idc += 1
            return block
            ...
            raise Exception(
                f"Error, an error occurred in remove vec for locals in_specification: {e}"
            )

    def replace_unsupported_function_with_manual_loop(
        self, assignment_stmt: F23.Assignment_Stmt
    ):
        """
        Replace unsupported GPU functions (e.g., MINLOC, MAXLOC)
        in a Fortran assignment with a manual loop.

        Parameters
        ----------
        assignment_stmt : F23.Assignment_Stmt
            The Fortran assignment statement to process.

        Returns
        -------
        str
            The modified Fortran code with unsupported functions replaced by a manual loop.
        """
        func_node = next(
            (
                child
                for child in walk(assignment_stmt)
                if isinstance(child, F23.Intrinsic_Function_Reference)
            ),
            None,
        )
        if not func_node:
        ...
        for child in func_node.children[1].children:
            if (
                isinstance(child, F23.Actual_Arg_Spec)
                and child.children[0].tostr().lower() == "dim"
            ):
    ...
    def replace_vec_colon_with_index(self, assignment_stmt: F23.Assignment_Stmt):
        """
        ...
        -------
        F23.Assignment_Stmt
            A new assignment statement with colons replaced by
            index variables.
        """
        try:
            assert isinstance(assignment_stmt, F23.Assignment_Stmt), (
                ...
                if ":" in node.tostr() and isinstance(node, F23.Part_Ref):
                    ...
                    if node_in_intrinsic:
                        for intrinsic_parent in intrinsic_name.keys():
                            reduction_dim = "ALL"
                            print(
                                f"node {node.tostr()} found within intrinsic parent {intrinsic_parent}"
                            )
                            if "dim" in intrinsic_name[intrinsic_parent]:
                                old_arg_spec = intrinsic_name[intrinsic_parent]["dim"]
                                reduction_dim = old_arg_spec.split("=")[-1].strip()
                                print(
                                    f"{reduction_dim}th of :s in {node.tostr()} must stay as < : >."
                                )
                                reduction_dim = self.process_section_subscript_list(
                                    node, reduction_dim
                                )
                                print(
                                    f"dimension {reduction_dim} of {node.tostr()} must stay as < : >."
                                )
                            mod_node, new_arg_spec = self.modify_colon_array_vec(
                                node, intrinsic_parent, reduction_dim
                            )
                            mod_intrinsic_parent = re.sub(
                                re.escape(node.tostr()),
                                mod_node.tostr(),
                                intrinsic_parent,
                            )
                            if new_arg_spec is not None:
            ...
            if self.contains_unsupported_function(output_stmt):
                print(
                    f"Unsupported function found in assignment statement: {output_stmt.tostr()}"
                )
                output_stmt = self.replace_unsupported_function_with_manual_loop(
                    output_stmt
                )
            return output_stmt
    ...
    def create_act_call_stmt(self, subroutine_stmt: F23.Subroutine_Stmt) -> None:
        """
        ...
        Parameters
        ----------
        subroutine_stmt : F23.Subroutine_Stmt
            The subroutine statement node to process.

        Returns
        -------
        None
            Updates the `subroutine_call_act_vec` attribute with the constructed statement.
    ...
    def merge_vector_loop(self, block: F23.Block) -> F23.Block:
        ...
        ----------
        block : F23.Block
            The block of code containing various statements.

        Returns
        -------
        F23.Block
            The modified block of code with merged vector loops.
        """
        try:
            if hasattr(block, "content"):
                idc = 0
                while idc < len(block.content):
                    ...
                    if isinstance(child, F23.Subroutine_Stmt):
                        ...
                        if self.dummy_add:
                            add_decl = (
                                f"INTEGER(KIND = i_std), INTENT(IN) :: {self.dummy_add}"
                            )
                            ...
                            if isinstance(grandchild, F23.Name):
                                ...
                                for grandgrandchild in grandchild.children:
                                    ...
                                    if grandgrandchild_str in self.all_array_info:
                                        self.acc_enter_data_copyin.append(
                                            grandgrandchild_str
                                        )
                                if self.error_flag.keys():
                                    error_flag_keys = ", ".join(
                                        [name for name in self.error_flag.keys()]
                                    )
                                    subroutine_stmt += (
                                        f"({error_flag_keys}, {dummy_arg_list_new})"
                                    )
                                else:
                                    subroutine_stmt += f"({dummy_arg_list_new})"
                            else:
                                raise ValueError(
                                    f"Unexpected type '{type(grandchild)}' encountered in children."
                                )
                        assert subroutine_name is not None, (
                            "Subroutine name cannot be None."
                        )
                        ...
                        acc_routine_seq_cmd = Processor().parse_fortran_comment(
                            "!$ACC ROUTINE SEQ"
                        )
                            ...
                            for grandchild in child.children:
                                ...
                                if isinstance(grandchild, F23.Name):
                                    ...
                                    if (
                                        ...
                                        and grandchild.tostr() in self.child_error_flag
                                        and self.child_error_flag[grandchild.tostr()]
                                    ):
                                        add_error_flag_into_call_stmt = True
                                        add_error_flags = self.child_error_flag[
                                            grandchild.tostr()
                                        ]
                                    ...
                                    if add_error_flag_into_call_stmt:
                                        error_flag_keys = ", ".join(
                                            [name for name in add_error_flags.keys()]
                                        )
                                        call_stmt += f"({error_flag_keys}, {self.dummy_add}, {grandchild.tostr()})"
                                    else:
                                        call_stmt += (
                                            f"({self.dummy_add}, {grandchild.tostr()})"
                                        )
                                else:
                                    raise ValueError(
                                        f"Unexpected type '{type(grandchild)}' encountered in children."
                        ...
                        block.content[idc] = F23.End_Subroutine_Stmt(
                            end_subroutine_stmt
                        )
                    elif isinstance(child, F23.Specification_Part):
                        new_specification_part = self.modify_specification_part(child)
                        block.content[idc] = (
                            self.remove_vec_for_locals_in_specification(
                                new_specification_part
                            )
                        )
                        ...
                        self.add_dos(stmt[0])
                        self.replace_where(child)
                        block.content[idc] = Processor().parse_fortran_statement(
                            self.code
                        )
                        ...
                        child, F23.Open_Stmt | F23.Write_Stmt | F23.Close_Stmt
                    ):
                        ...
                        if isinstance(block.content[idc + 2], F23.End_If_Stmt):
                            ...
                            raise RuntimeError(
                                "Error: The next statement is not an End_If_Stmt. "
    ...
    def add_vector_loop(self, block: F23.Block) -> F23.Block:
        """
        ...
        ----------
        block : F23.Block
            The block of code where the vector loop should be added.

        Returns
        -------
        F23.Block
            The modified block of code with the vector loop added.
        """
        try:
    ...
    def replace_gpu_unsupported(self, subroutine_tree):
        ...
        Parameters
        ----------
        subroutine_tree : Program
            The parse tree of the Fortran subroutine.

        Returns
        -------
        tuple
            Updated subroutine_tree and a dictionary containing error flag information.
        """
        try:
            if self.contains_unsupported_function(subroutine_tree):
                self.minmax_value_str, self.minmax_index_str = (
                    "minmax_value",
                    "minmax_index",
                )
                self.gpu_unsupported_decl = [
                    F23.Type_Declaration_Stmt(
                        f"INTEGER(KIND = i_std) :: {self.minmax_index_str}"
                    ),
                    F23.Type_Declaration_Stmt(
                        f"REAL(r_std) :: {self.minmax_value_str}"
                    ),
                ]
            ...
            subroutine_name = next(
                (
                    child.children[0].get_name().string
                    for child in subroutine_tree.children
                    if isinstance(child, F23.Subroutine_Subprogram)
                ),
                None,
            )
            ...
            for parent_str, writes in parent_to_writes.items():
                ...
                if isinstance(parent.children, tuple):
                    continue
                error_flag_decl_str = (
                    f"INTEGER(KIND = i_std), INTENT(INOUT) :: {error_flag_name}"
                )
                ...
                dict_info["write_calls"] = Processor().parse_fortran_statement(
                    check_point
                )
