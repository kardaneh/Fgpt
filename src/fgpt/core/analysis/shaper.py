...
class Shaper:
    ...
    def __init__(
        ...
        module_path: str,
        dummy_arg_list: defaultdict,
        actual_arg_spec_list: defaultdict = None,
        call_subroutines: defaultdict = None,
        logger: Logger = None,
    ) -> None:
    ...
    def find_fortran_files_subroutine(self, subroutine_key: str) -> None:
        """
        ...
        try:
            ...
            while fortran_file_queue_imp:
                ...
                if module_name in self.parsed_modules:
                    ...
                    self.module_tree_imp = self.processor.parse_fortran_file(
                        module_file_path
                    )
                ...
                for sub in walk(self.module_tree_imp, F23.Subroutine_Subprogram):
                    ...
                    assert subroutine_name is not None, (
                        f"Unexpected type {subroutine_name} encountered in children."
                    )
                    check = all(
                        case not in subroutine_name for case in self.cases_to_exclude
                    )
                    if not check:
                        continue
                    ...
                    if arg_list is not None:
                        if subroutine_name not in self.dummy_arg_list:
                            for child in arg_list.children:
                                self.dummy_arg_list[subroutine_name].append(
                                    child.tostr()
                                )
                        ...
                        for item in call_stmt:
                            ...
                            if call_name == subroutine_key:
                                ...
                                if arg_list:
                                    arg_string = [
                                        string.strip()
                                        for string in arg_list[0].tostr().split(",")
                                    ]
                                    self.actual_arg_spec_list[call_name].append(
                                        arg_string
                                    )
            ...
            self.processor.logger.error(
                f"Subroutine '{subroutine_key}' not found in any Fortran files within {self.module_dir_imp}"
            )
            raise

        except Exception as e:
            self.processor.logger.exception(
                "An error occurred while searching for Fortran files in method 'find_fortran_files_subroutine': ",
                e,
            )
            raise

    def shaper_subroutine(
        self, node: F23.Type_Declaration_Stmt, subroutine_key: str
        ...
        """
        try:
            assert self.actual_arg_spec_list is not None, (
            ...
            for arg, call in zip(
                self.actual_arg_spec_list[subroutine_key],
                self.call_subroutines[subroutine_key],
            ):
                ...
                if declaration_part:
                    for decl in walk(declaration_part, F23.Type_Declaration_Stmt):
                        ...
                        if act_arg in declarations:
                            ...
                            if explicit_shape:
                                self.processor.logger.info(
                                    f"Found explicit shape in subroutine '{subroutine_key}'!"
                                )
                                return decl
                            else:
            ...
            self.finder = Navigator(
                self.module_dir_imp,
                self.module_tree_imp,
                self.parsed_modules,
                self.module_path,
                logger=self.logger,
            )
            self.finder.variable_finder(act_arg)
            return self.processor.combine_allocate_declaration(
                self.finder.var_declaration
            )

        except Exception as e:
            self.processor.logger.exception(
                "An error occurred in method 'shaper_subroutine': ", e
            )
            raise

    def shaper_function(
        self,
        node: F23.Type_Declaration_Stmt,
        function_tree: F23.Function_Subprogram,
        function_key: str,
        all_array_info: dict,
    ) -> F23.Type_Declaration_Stmt:
        """
            ...
            while function_assignment_stmt is None:
                for assignment_stmt in walk(self.module_tree_imp, F23.Assignment_Stmt):
                    for part_ref in walk(assignment_stmt, F23.Part_Ref):
                        if (
                            isinstance(part_ref.children[0], F23.Name)
                            and part_ref.children[0].tostr() == function_key
                        ):
                            function_assignment_stmt = True
                            assert isinstance(
                                part_ref.children[1], F23.Section_Subscript_List
                            ), (
                                f"Expected Section_Subscript_List as second child, got {type(part_ref.children[1])}"
                            )
                        ...
                        #            act_arg_list = child
                        if function_assignment_stmt:
                            act_arg = act_arg_list.children[
                                self.dummy_arg_list[function_key].index(name)
                            ]
                            # print(act_arg, "llllllllllllll======")
                            if ":" in act_arg.tostr() and isinstance(
                                act_arg, F23.Part_Ref
                            ):
                            ...
                            new_dec = self.processor.map_declaration(
                                node, explicit_dec=None, dimensions=dimensions
                            )
                            break
                    if function_assignment_stmt:
                        break
    ...
    def shaper_intrinsic_size(
        self, node: F23.Type_Declaration_Stmt
    ) -> F23.Type_Declaration_Stmt:
        """
        ...
        try:
            ...
            for explicit_shape in walk(node, F23.Explicit_Shape_Spec):
                ...
                if not intrinsics:
                    ...
                    if intrinsic.tostr() == "SIZE":
                        ...
                        if len(args) == 2:
                            if isinstance(args[1], F23.Actual_Arg_Spec):
                                ...
                                if dim_key.tostr().lower() == "dim":
                                    ...
                                    ), "Second item in must be an Int_Literal_Constant."
                        ...
                        for declaration_stmt in walk(
                            node.parent, F23.Type_Declaration_Stmt
                        ):
                            ...
                            if entity_decls[0].tostr() == args[0].tostr():
                                size = "1"
                                for idim, explicit_shape in enumerate(
                                    walk(declaration_stmt, F23.Explicit_Shape_Spec),
                                    start=1,
                                ):
                                    parts = [
                                        part.strip()
                                        for part in explicit_shape.tostr().split(":")
                                    ]
                                    if len(parts) == 2:
                                        dim_size = (
                                            "(" + parts[1] + "-" + parts[0] + "+1" + ")"
                                        ...
                                        self.processor.logger.error(
                                            f"Unexpected number of parts in explicit shape: {len(parts)}. Parts: {parts}"
                                        )
                                        raise
                                    if dim_value != "ALL":
                        ...
                        self.processor.logger.error(
                            f"Unexpected intrinsic function: {intrinsic.tostr()}"
                        )
                        raise
            dimensions = ", ".join([name for name in shape])
            new_dec = self.processor.map_declaration(
            ...
            for igc, gchild in enumerate(node.parent.children):
                if isinstance(gchild, F23.Type_Declaration_Stmt):
                    entity_decl = walk(gchild, F23.Entity_Decl)[0].tostr()
                    if entity_decl == name:
                        node.parent.children[node.parent.children.index(gchild)] = (
                            new_dec
                        )
            return new_dec
        except Exception as e:
            self.processor.logger.exception(
                "An error occurred in method 'find_enclosing_subroutine': ", e
            )
            raise

    def find_enclosing_subroutine(
        self, node: F23.Base
    ) -> F23.Subroutine_Subprogram | None:
        """
