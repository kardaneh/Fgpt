...
class _ConditionalLowering:
    ...
    def _handle_lax_cond(
        ...
        try:
            ...
            for name in assigned:
                if (
                    self._chain_pre_existing.get(name, False)
                    and name not in used_after
    ...
    def visit_If(self, node: ast.If):
        ...
        try:
            ...
            for name in assigned:
                if (
                    name in self._seen_assigned_names
                    and name not in self._chain_pre_existing
                ):
                    self._chain_pre_existing[name] = name in self._seen_assigned_names
                    newly_tracked.append(name)
            try:
                ...
                if cond_type in ["masked"]:
                    ...
                    has_break_in_orelse = any(
                        isinstance(s, ast.Break) for s in node.orelse
                    )
                ...
                for name in newly_tracked:
                    del self._chain_pre_existing[name]
