...
from fgpt.core.backends.utils import Control, get_name
...
class _LoopLowering:
    ...
    def visit_For(self, node: ast.For) -> ast.AST | list[ast.AST] | None:
        ...
        try:
            ...
            if len(range_args) == 1:
                ...
                raise ValueError(
                    f"range() expects 1-3 arguments, got {len(range_args)}"
                )
            ...
            if cond_type == "index_loop":
                ...
                self._push_scope()
                self._vector_loop_stack.append(loop_index)
                try:
    ...
    def handle_scan(
        ...
        * :attr:`_modified_ret_stack`
        ...
        try:
            ...
            existing = {name.removeprefix("self.") for name in inputs}

            for name in set(assigned) & self._seen_assigned_names:
                base = name.removeprefix("self.")

                if (
                    name not in used_after
                    and base not in existing
                    and name not in self._always_exclude
                ):
                    inputs.append(name)
                    existing.add(base)
    ...
    def visit_While(self, node: ast.While) -> list[ast.AST]:
        ...
        try:
            ...
            if is_vector:
                # Scalarized path: WhileVectorToScalar will rewrite x.at[idx].set(expr) -> x_scalar = expr,
                # so the initial scalar value is exactly the RHS the stack-walk already found.
                pass  # keep intial_values as-is
            else:
                # Non-scalarized path: any variable that is itself already a bound array
                # in the enclosing/carry scope must keep referencing that array,
                # not a partial subscript-assignment's RHS.
                for name in state_inputs:
                    if name in self._scan_stack[-1].get("carry", []):
                        intial_values[name] = ast.Name(id=name, ctx=ast.Load())
