...
from fgpt.core.backends.utils import Control, contains_name, get_name
...
class _DynamicLoop:
    ...
    def make_depth_index(self, node: ast.AST) -> tuple[ast.Assign | None, bool]:
        """
        ...
        try:
            ...
            if (
                ...
                and target_node.id not in self.dynamic_variable_lift
            ):
                self.dynamic_created_variables["depth_idx"] = get_name(target_node)
            ...
            depth_assign = ast.Assign(
                ...
                value=ast.Call(
                    func=ast.Attribute(
                        value=target_node, attr="astype", ctx=ast.Load()
                    ),
                    args=[
                        ast.Attribute(
        ...
        try:
            ...
            for stmt in node.body:
                if isinstance(stmt, ast.Assign):
                    ...
                    all_assign_nodes.extend(
                        s for s in stmt.body if isinstance(s, ast.Assign)
                    )
            ...
            if start_depth_index is not None:
                start_depth_index.targets[0] = ast.Name(
                    id="depth_idx_start", ctx=ast.Store()
                )
                ...
                if isinstance(start, ast.Call):
                    start = start.args[0]
                self._inferred_ranks["depth_idx_start"] = ranks.get(start, 0) or 0
            ...
            if isinstance(stop, ast.Call):
                stop = stop.args[0]
            self._inferred_ranks["depth_idx"] = ranks.get(stop, 0) or 0
            ...
            pushed_control = False
            old_metadata = None

            if vectorization_context is not None:
                if not vectorization_context.get("metadata"):
            ...
            if (
                pushed_control
                and self._control_stack
                and self._control_stack[-1].kind == "loop"
            ):
    ...
    def _transform_conditional_search(
        ...
        vect_axes: set[int],
        start: ast.AST,
        upper_bound_static: ast.AST | None,
        vectorization_context: dict | None,
        layer_value: ast.AST | None,
        ...
        try:
            ...
            has_real_vectorization = bool(
                vectorization_context
                and vectorization_context.get("vectorization_axis")
            )

            if not has_real_vectorization:
                cond_node = self._substitute_name(
                    copy.deepcopy(cond_node), loop_index, "layers"
                )
    ...
    def _vectorize_condition(
        ...
        try:
            if isinstance(test_node, ast.BoolOp):
                jnp_func = (
                    "logical_and" if isinstance(test_node.op, ast.And) else "logical_or"
                )
                vectorized_values = [
                    self._vectorize_condition(
                        v, loop_index, loop_axis, vect_axes, vectorization_context
                    )
            ...
            elif isinstance(test_node, ast.UnaryOp) and isinstance(
    ...
    def _vectorize_compare(
        ...
        try:
            ...
            visited_comparators = [
                self.visit(copy.deepcopy(c)) for c in compare_node.comparators
            ]

            target_rank = loop_axis + 1

            aligned_left, _ = self.maybe_add_index(
                visited_left, target_rank, vectorization_context
            )
            aligned_comparators = []
            for comp in visited_comparators:
                aligned_comp, _ = self.maybe_add_index(
                    comp, target_rank, vectorization_context
                )
    ...
    def _emit_masked_search_assign(
        ...
        try:
            ...
            rhs_with_found = self._substitute_name(
                copy.deepcopy(rhs_node), loop_index, "_found_layer"
            )
            ...
            any_active = ast.Call(
                func=ast.Attribute(
                    value=ast.Name(id="jnp", ctx=ast.Load()), attr="any", ctx=ast.Load()
                ),
                args=[ast.Name(id="_loop_active", ctx=ast.Load())],
                keywords=[ast.keyword(arg="axis", value=ast.Constant(value=loop_axis))],
            )
        ...
        try:
            ...
            if isinstance(node, ast.Subscript):
                ...
                if arr_name is not None and arr_name in self.dynamic_variable_lift:
                    return True
                # not a lifted array, but still a runtime subscript -> dynamic
                return True
            return False
        except Exception as e:
    ...
    def _resolve_static_upper_bound(
        ...
        loop_index: str,
        vectorization_context: dict | None,
        condition_nodes: list[ast.AST] | None = None,
    ) -> ast.AST | None:
        """
                ...
                for sub in ast.walk(assign_node.value):
                    if isinstance(sub, ast.Subscript):
                        ...
                        for pos, idx_node in enumerate(sub_indices):
                            if contains_name(idx_node, loop_index) and pos < len(
                                node_dims
                            ):
                                dim = node_dims[pos]
                                attributes = self.cls_info[self.cls_name].get(
    ...
    def _detect_batch_dimension(
        ...
        try:
            ...
            vectorization_axis = (vectorization_context or {}).get(
                "vectorization_axis", {}
            )
            ...
            for assign_node in assign_nodes:
                ...
                node_dims = self.get_declared_dims(lhs)
                subscript_indices = (
                    lhs.slice.elts if isinstance(lhs.slice, ast.Tuple) else [lhs.slice]
                )
                ...
                loop_dims = {
                    node_dims[pos] for pos in loop_positions if pos < len(node_dims)
                }
    ...
    def _build_layers_mask(
        ...
        stop: ast.AST,
        step: ast.AST,
        depth_index: ast.Assign | None,
        vect_present: bool,
        upper_bound_static: ast.AST | None,
        assign_nodes: list[ast.Assign],
        loop_index: str,
        vectorization_context: dict | None,
        start_depth_index: ast.Assign | None = None,
        start_vect_present: bool = False,
        condition_nodes: list[ast.AST] | None = None,
    ) -> tuple[ast.Assign | None, ast.Assign, int, set[int]]:
        """
        ...
        try:
            ...
            if isinstance(step, ast.UnaryOp):
                arange_start = ast.BinOp(
                    left=upper_bound_static, op=ast.Sub(), right=step.operand
                )
                arange_stop = ast.Constant(value=-1)
            else:
                arange_start = (
                    start
                    if has_batch_dimension and not start_vect_present
                    else ast.Constant(0)
                )
            ...
            if target_node is not None:
                if isinstance(target_node, ast.Subscript):
                    ...
                    if target_node.id in self.dynamic_variable_lift:
                        ...
                        target_node.id in self.var_state
                        and self.var_state.get(var_name)[0] == "stateful"
                    ):
            ...
            if not vect_axes:
                ...
                if start_depth_index is not None:
                    ...
                    lower_cmp = ast.Compare(
                        left=layers_node, ops=[ast.GtE()], comparators=[start]
                    )
                else:
                    lower_cmp = None
                ...
                mask_assign = ast.Assign(
                    targets=[ast.Name(id="mask", ctx=ast.Store())], value=mask_val
                )
            ...
            upper_cmp = ast.Compare(
                left=layers_subscript,
                ops=[ast.Lt()],
                comparators=[depth_index_subscript],
            )
            ...
            if start_depth_index is not None:
                ...
                lower_cmp = ast.Compare(
                    left=layers_subscript,
                    ops=[ast.GtE()],
                    comparators=[start_subscript],
                )
            elif not has_batch_dimension and isinstance(start, ast.Constant):
                lower_cmp = ast.Compare(
                    left=layers_subscript, ops=[ast.GtE()], comparators=[start]
                )
            else:
                lower_cmp = None
    ...
    def _lhs_contains_loop_index(self, lhs_node: ast.AST, loop_index: str) -> bool:
        ...
        try:
            ...
            indices = (
                lhs_node.slice.elts
                if isinstance(lhs_node.slice, ast.Tuple)
                else [lhs_node.slice]
            )
    ...
    def _transform_elementwise(
        ...
        try:
            ...
            loop_info = (vectorization_context or {}).get("loop_info", {})
            vectorization_axis_map = (vectorization_context or {}).get(
                "vectorization_axis", {}
            )
            ...
            if not has_real_vectorization:
                lhs_node = self._substitute_name(
                    copy.deepcopy(lhs_node), loop_index, "layers"
                )
                rhs_node = self._substitute_name(
                    copy.deepcopy(rhs_node), loop_index, "layers"
                )
                assign_node = ast.Assign(targets=[lhs_node], value=rhs_node)
                if vectorization_context is not None:
            ...
            value, temp = self.maybe_add_index(
                rhs_node, target_rank, vectorization_context
            )
            ...
            if vectorization_context and vectorization_context.get("metadata"):
                vectorization_context["metadata"] = old_metadata
    ...
    def _extract_set_payload(
        self, assign_node: ast.Assign, target_name: str
    ) -> ast.AST | None:
        """
    ...
    def _transform_accumulation(
        ...
        *loop_index* — a reduction over the dynamic loop's range (e.g.
        ...
        try:
            ...
            reduction_term = self._extract_reduction_term(
                payload, accum_base, loop_index
            )

            if start_is_dynamic:
                reduction_term = self._substitute_loop_index_with_offset(
                    reduction_term, loop_index, has_batch_dimension=array_has_batch_axis
                )
            ...
            sum_call = ast.Call(
                func=ast.Attribute(
                    value=ast.Name(id="jnp", ctx=ast.Load()), attr="sum", ctx=ast.Load()
                ),
                args=[where_call],
                keywords=[ast.keyword(arg="axis", value=ast.Constant(value=loop_axis))],
            )
    ...
    def _substitute_loop_index_with_offset(
        self, node: ast.AST, loop_index: str, has_batch_dimension: bool = False
    ) -> ast.AST:
        """
        ...
        has_batch_dimension : bool
            Defines if there's a batched dimension due to vectorization
                    ...
                    if (
                        ...
                        if has_batch_dimension:
                            n.left = ast.Subscript(
                                value=ast.Name(id="layers", ctx=ast.Load()),
                                slice=ast.Tuple(
                                    elts=[ast.Constant(value=None), ast.Slice()],
                                    ctx=ast.Load(),
                                ),
                                ctx=ast.Load(),
                            )
                            n.right = ast.Subscript(
                                value=ast.Name(id="depth_idx_start", ctx=ast.Load()),
                                slice=ast.Tuple(
                                    elts=[ast.Slice(), ast.Constant(value=None)],
                                    ctx=ast.Load(),
                                ),
                                ctx=ast.Load(),
                            )
                        else:
                            n.left = ast.Name(id="layers", ctx=ast.Load())
                            n.right = ast.Name(id="depth_idx_start", ctx=ast.Load())
                    return n
    ...
    def _extract_reduction_term(
        self, rhs_node: ast.AST, lhs_node: ast.AST, loop_index: str
    ) -> ast.AST:
        """
        ...
        try:
            ...
            def _strip(node):
                if isinstance(node, ast.BinOp) and isinstance(node.op, ast.Add):
                    left_src = ast.unparse(
                        ast.fix_missing_locations(copy.deepcopy(node.left))
                    )
                    right_src = ast.unparse(
                        ast.fix_missing_locations(copy.deepcopy(node.right))
                    )
                    if left_src == lhs_src:
                        return node.right
    ...
    def _extract_accum_base_from_payload(
        self, payload: ast.AST, lhs_visited: ast.AST
    ) -> ast.AST:
        """
        ...
        *lhs_visited*'s array name.
        ...
        """
        try:
            if not isinstance(payload, ast.BinOp) or not isinstance(
                payload.op, ast.Add
            ):
                return lhs_visited
    ...
    def _rhs_is_accumulation(self, rhs_node: ast.AST, lhs_node: ast.AST) -> bool:
        ...
        try:
            ...
            lhs_src = ast.unparse(ast.fix_missing_locations(copy.deepcopy(lhs_node)))
            left_src = ast.unparse(
                ast.fix_missing_locations(copy.deepcopy(rhs_node.left))
            )
            right_src = ast.unparse(
                ast.fix_missing_locations(copy.deepcopy(rhs_node.right))
            )
