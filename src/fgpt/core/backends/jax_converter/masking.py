...
from fgpt.core.backends.utils import get_name
...
class _Masking:
    ...
    def normalize_masked_node(
        ...
        try:
            if isinstance(node, ast.Subscript):
                ...
                node.args = [
                    self.normalize_masked_node(arg, mask_names)[0] for arg in node.args
                ]
                node.keywords = [
                    ast.keyword(
                        arg=kw.arg,
                ...
                node.values = [
                    self.normalize_masked_node(v, mask_names)[0] for v in node.values
                ]
                return node, None
    ...
    def handle_masked_where(self, node: ast.If) -> list[ast.Assign]:
        ...
        try:
            ...
            outer_mask_name = node.test.id if isinstance(node.test, ast.Name) else None
            ...
            def build_block(block):
                ...
                if len(block) >= 1:
                    for stmt in block:
                        ...
                        if isinstance(target, ast.Name):
                            ...
                            if isinstance(
                                target.slice, ast.Compare | ast.Subscript | ast.BoolOp
                            ):
                                found_masked_write = True
                            elif isinstance(target.slice, ast.Name):
                                if (
                                    target.slice.id in mask_names
                                    or target.slice.id == outer_mask_name
                                ):
                                    found_masked_write = True

                        if found_masked_write:
                            if isinstance(target, ast.Subscript):
                                target, _ = self.normalize_masked_node(
                                    target, mask_names
                                )

                            value, _ = self.normalize_masked_node(
                                stmt.value, mask_names
                            )
                ...
                raise NotImplementedError(f"Unsupported WHERE block structure: {block}")
        ...
        try:
            ...
            if not self.is_arr_at_op_call(value, target_name):
                if isinstance(assign, ast.Assign) and self._control_stack:
                    if not self.check_if_array(assign) or (
                        target_name in self.var_state
                    ):
                        if self._is_control_temporary(
                            target_name, assigned, used_after
                        ):
            ...
            arr_name = (
                at_sub.value.value.id
                if isinstance(at_sub.value.value, ast.Name)
                else at_sub.value.value.attr
            )
            ...
            if (self.counter == 0 and self.for_counter == 0) and self.cls_info[
                self.cls_name
            ].get("attributes").get(arr_name):
                target_value = ast.Attribute(
                    value=ast.Name(id="self", ctx=ast.Load()),
                    attr=arr_name,
                    ctx=ast.Load(),
                )
                if isinstance(at_sub.value.value, ast.Name):
                    at_sub.value.value = ast.Attribute(
                        value=ast.Name(id="self", ctx=ast.Load()),
                        attr=arr_name,
                        ctx=ast.Load(),
                    )
            ...
            if isinstance(at_sub.slice, ast.Tuple):
                if all(isinstance(s, ast.Slice) for s in at_sub.slice.elts):
                    ...
                    old_val = ast.Subscript(
                        value=target_value, slice=at_sub.slice, ctx=ast.Load()
                    )
            else:
                old_val = ast.Subscript(
                    value=target_value, slice=at_sub.slice, ctx=ast.Load()
            ...
            rhs_expr, elts_list = self.maybe_add_index(
                rhs_expr, target_rank, vect_context
            )
            ...
            if elts_list:
                if (
                    vect_context
                    and vect_context.get("metadata", {}).get("current_mask_rank", 0)
                    == 0
                ):
                    pass
                else:
                ...
                if target_rank > 1 and vect_context:
                    current_mask_rank = vect_context.get("metadata", {}).get(
                        "current_mask_rank", 0
                    )
            ...
            if op_name != "set" and isinstance(rhs_expr, ast.BinOp):
                ...
                return [
                    ast.fix_missing_locations(delta_value),
    ...
    def build_masked_update(
        ...
        if isinstance(target, ast.Subscript):
            ...
            return ast.Assign(targets=[base], value=at_call)
        ...
        # Case 2: simple variable (A = ...)
            ...
            return ast.Assign(targets=[lhs], value=where_call)
        ...
        try:
            ...
            # Case: cleanup arr[:][:] → arr[:]
                ...
                node.targets[0] = ast.Subscript(
                    value=base, slice=new_slice, ctx=ast.Store()
                )
    ...
    def _flatten_subscript(self, sub: ast.Subscript) -> tuple[ast.AST, list[ast.AST]]:
        """
    ...
    def _effective_remaining_dims(
        self, sub_or_expr: ast.AST, declared_dims: list
    ) -> list:
        """
        Determine the dimensions that remain after applying a subscript operation.

        Given a subscript expression and a list of declared dimensions, this
        method removes dimensions that are consumed by scalar (non-slice)
        indices while preserving dimensions corresponding to slice indices.
        If the provided expression is not an ``ast.Subscript``, the original
        list of declared dimensions is returned unchanged.

        Parameters
        ----------
        sub_or_expr : ast.AST
            The AST node to examine. If this is an ``ast.Subscript``, its
            indexing operation is analyzed to determine which dimensions are
            retained. Otherwise, no analysis is performed.
        declared_dims : list
            The list of declared dimensions associated with the indexed object,
            in the same order as the array axes.

        Returns
        -------
        list
            A list containing only the dimensions that remain after indexing.
            Dimensions indexed by scalar values (e.g., integer constants or
            variables) are removed, while dimensions indexed by slices are
            retained.

        Notes
        -----
        This method assumes that scalar indices reduce the rank of the array,
        whereas slice indices preserve the corresponding dimension. Only the
        dimensions covered by the provided indices are considered.
        """
        if not isinstance(sub_or_expr, ast.Subscript):
            return declared_dims

        idx_node = sub_or_expr.slice
        idx_elts = idx_node.elts if isinstance(idx_node, ast.Tuple) else [idx_node]

        remaining = []
        for dim, idx in zip(declared_dims, idx_elts):
            if isinstance(idx, ast.Slice):
                remaining.append(dim)
            # else: scalar index (ji, constant, etc.) consumes this dim — drop it
        return remaining
    ...
    def create_mask(self, axis: int, sub: ast.Subscript, mask_name: str):
        ...
        try:
            if not isinstance(sub, ast.Subscript):
                raise ValueError(
                    f"The given argument is not that of ast.Subscript \
                                , {type(sub)}"
                )
            new_stmts = []
            ...
            node_dim = self._effective_remaining_dims(sub, self.get_declared_dims(sub))
            ...
            upper_dims = (
                self._effective_remaining_dims(upper, self.get_declared_dims(upper))
            ...
            lower_dims = (
                self._effective_remaining_dims(lower, self.get_declared_dims(lower))
            ...
            if lower_expr is not None:
                lower_cmp = ast.Compare(
                    left=idx_expr, ops=[ast.GtE()], comparators=[lower_expr]
                )
            else:
                lower_cmp = None
            ...
            if upper_expr is not None:
                upper_cmp = ast.Compare(
                    left=idx_expr, ops=[ast.Lt()], comparators=[upper_expr]
                )
            else:
                upper_cmp = None

            if lower_cmp and upper_cmp:
                mask_expr = ast.BinOp(left=lower_cmp, op=ast.BitAnd(), right=upper_cmp)
            elif upper_cmp:
                mask_expr = upper_cmp
            else:
                mask_expr = lower_cmp
            ...
            mask_assign = ast.Assign(
                targets=[ast.Name(id=mask_name, ctx=ast.Store())], value=mask_expr
            )
    ...
    def remove_dynamic_slice(
        self, axis: int, subs: list[tuple[ast.Subscript, ast.Slice]]
    ) -> None:
        """
    ...
    def wrap_with_slice_specific_masks(
        self, expr: ast.AST, mask_map: dict[int, str]
    ) -> ast.AST:
        """
        ...
        try:
            if isinstance(expr, ast.Subscript):
                ...
                expr.args = [
                    self.wrap_with_slice_specific_masks(arg, mask_map)
    ...
    def handle_dynamic_slice(
        ...
        try:
            ...
            # Step 4: wrap RHS
            node.value = self.wrap_with_slice_specific_masks(
                node.value, subscript_to_mask
            )
    ...
    def get_dynamic_slices(
        self, node: ast.AST, inside_reduction: bool = False
    ) -> list[ast.Subscript]:
        """
        ...
        * ``nanmean``
        ...
        try:
            ...
            is_reduction = (
                ...
                and node.func.attr
                in {"sum", "mean", "max", "min", "prod", "nansum", "nanmean"}
            )
                ...
                if inside_reduction:
                    ...
                    # Case 2: multidimensional a[i, 1:n]
                        ...
                        for elt in slice_node.elts:
                            if isinstance(elt, ast.Slice) and self._is_dynamic_slice(
                                elt
                            ):
                                results.append(node)
                                break
            ...
            for child in ast.iter_child_nodes(node):
                results.extend(
                    self.get_dynamic_slices(
                        child, inside_reduction=inside_reduction or is_reduction
                    )
                )
    ...
    def expand_to_full_shape(self, expr: ast.AST, expr_dims, target_dims):
        ...
        try:
            ...
            return ast.Subscript(
                value=expr, slice=ast.Tuple(elts=elts, ctx=ast.Load()), ctx=ast.Load()
            )
        except Exception as e:
