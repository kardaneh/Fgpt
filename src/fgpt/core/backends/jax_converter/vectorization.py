...
class _Vectorization:
    ...
    def _target_rank(self, slice_node: ast.AST) -> int:
        ...
        axis. Falls back to ``0`` when no control context is active
        (a bare scalar target outside vectorisation still occupies zero
        ...
        *target_rank*.
        ...
        try:
            ...
            transformer = MaybeAddIndexTransformer(
                ...
                vect_context=vect_context if vect_context else {},
    ...
    def _apply_broadcasting_slice(self, sub: ast.Subscript, assign_vect: dict):
        ...
        try:
            if isinstance(sub.slice, tuple):
                new_slices = []
                for s in sub.slice:
                    if isinstance(s, ast.Name) and s.id in assign_vect.get(
                        "vectorization_axis", {}
                    ):
                ...
                if isinstance(sub.slice, ast.Name) and sub.slice.id in assign_vect.get(
                    "vectorization_axis", {}
                ):
                    sub.slice = ast.Slice()
                    return ast.Slice()
    ...
    def _expr_depends_on_axes(self, node: ast.AST, vectorization_axis: dict) -> tuple:
        ...
        try:
            ...
            visit(node)
            ordered = tuple(ax for ax in (vectorization_axis or []) if ax in deps)
    ...
    def _infer_scalar_shape(self, vectorization_context: dict) -> tuple:
        ...
        try:
            ...
            return tuple(
                ...
                for axis in self._vector_loop_stack
            )
        except Exception as e:
    ...
    def _process_lift(
        ...
        try:
            ...
            dtype = local_arrays[array_name].get("dtype", "float64")
    ...
    def _emit_convergence_break(
        ...
        try:
            ...
            converged_name = self._get_or_create_converged_var(
    ...
    def _get_or_create_converged_var(self, vectorization_context: dict | None) -> str:
        ...
        The sizing dimension ``<dim>`` is resolved dynamically rather
        than assumed: the innermost active vectorized loop variable is
        read from :attr:`_vector_loop_stack`, then matched against
        ``vectorization_context['loop_info']`` (a ``{dim: loop_var}``
        mapping populated by :meth:`_setup_vector_control`) to find the
        dimension attribute associated with that loop. This correctly
        handles nested vectorized loops over different dimensions (e.g.
        an outer loop over ``kjpindex`` containing an inner loop over
        ``klev``), always sizing the array against the dimension of the
        loop that is actually active at the point of creation.
        ...
        vectorization_context : Optional[dict]
            ...
            Must contain a ``'loop_info'`` mapping when a scan is
            active.
        ...
        Notes
        -----
        Resolution of the sizing dimension relies on
        :attr:`_vector_loop_stack` reflecting the innermost vectorized
        loop currently being lowered; this stack is pushed/popped
        around the body of each ``"vector"``-classified loop in
        :meth:`visit_For`.

        See Also
        --------
        :meth:`_setup_vector_control`
            Populates ``loop_info`` with ``{dim: loop_var}`` entries.

        :attr:`_vector_loop_stack`
            Tracks the loop variable of the innermost active vectorized
            loop.

        :meth:`handle_scan`
            Prepends ``ctx['converged_init']`` ahead of the generated
            ``lax.scan`` call.

        Raises
        ------
        ValueError
            If no vectorized loop is currently active
            (:attr:`_vector_loop_stack` is empty), if
            ``vectorization_context`` is missing or lacks
            ``'loop_info'``, or if the current loop variable cannot be
            matched to any dimension in ``loop_info``.
        ...
        try:
            ...
            if not self._vector_loop_stack:
                # raise ValueError(
                #     f"Cannot determine vectorization dimension for {name}: no active vector loop"
                # )
            ...
            current_loop_var = self._vector_loop_stack[-1]

            if not vectorization_context or "loop_info" not in vectorization_context:
                raise ValueError(
                    f"Cannot determine vectorization dimension for {name}: no loop context available"
                )

            loop_info = vectorization_context["loop_info"]
            vect_dim = next(
                (dim for dim, lv in loop_info.items() if lv == current_loop_var), None
            )

            if vect_dim is None:
                raise ValueError(
                    f"Loop variable {current_loop_var!r} not found in loop_info={loop_info!r}"
                )
