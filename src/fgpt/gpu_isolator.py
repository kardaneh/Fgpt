...
from fgpt.core.passes.modifier import Modifier
...
class Isolator:
    ...
    def __init__(self, rest_of_path, target_module, work, openacc):
        ...
        self.path_to_original = os.path.join(
            self.module_dir_sp, f"{self.target_module}_org.f90"
        )
    ...
    def isolate_procedure_function(self, cls, parent_procedure, child_function):
        ...
        shape_to_search = [
            var
            for var in cls.shapes_variables[child_function]
            if var.string not in scalar_names and var.string not in global_names
        ]
        if shape_to_search:
            cls.find_global_variables(
                self.module_dir_sp, self.module_tree_cp, shape_to_search, child_function
            )
            cls.var_global[child_function].extend(shape_to_search)

        cls.extract_array_info(
            cls.dec_global[child_function],
            cls.var_dummy[child_function],
            child_function,
        )
        ...
        for key, values in cls.dec_global[child_function].items():
            if walk(values, F23.Function_Subprogram):
                ...
                self.processor.logger.info(
                    f"Trying to isolate nested function .... {key}"
                )
                ...
                for i, call_site in enumerate(
                    cls.call_within_sub[child_function][nested_function], 1
                ):
                ...
                if nested_function in self.isolated_subroutines:
                    ...
                    self.isolate_procedure_function(
                        cls, child_function, nested_function
                    )

        child_functions = cls.call_within_sub[child_function].keys()
        if child_functions:
            self.processor.logger.info(
                f"Processing {len(child_functions)} function calls within parent function {child_function}"
            )
            for child_func in child_functions:
                self.collect_global_vars_decl(
                    cls.dec_global[child_func], cls.dec_global[child_function]
                )
        ...
        function_tree_cp = self.processor.parse_fortran_string(
            function_tree.tofortran()
        )
        ...
        call_stmt_org = F23.Assignment_Stmt(
            f"{cls.func_result[child_function]} = {child_function}({arg_list})"
        )
        ...
        error_status = self.processor.compile_and_run(
            os.getcwd(), self.target_module_dir
        )
    ...
    def isolate_child_subroutine(
        self, cls, parent_subroutine, child_subroutine, local_var_parent=None
    ):
        dummy_as_local = set()
        function_tree = None
        sub_trees = []
        ...
        self.processor.logger.info(
            f"Trying to isolate a child subroutine .... {child_subroutine}"
        )
        ...
        cls.find_global_variables(
            self.module_dir_sp,
            self.module_tree_cp,
            cls.var_global[child_subroutine],
            child_subroutine,
        )

        # Process declaration variables
        cls.process_declaration_variables(
            cls.var_dummy[child_subroutine], child_subroutine
        )
        for key in cls.dec_global[child_subroutine].keys():
            cls.process_declaration_variables(
                cls.dec_global[child_subroutine][key], child_subroutine
            )
        ...
        shape_to_search = [
            var
            for var in cls.shapes_variables[child_subroutine]
            if var.string not in scalar_names and var.string not in global_names
        ]
        if shape_to_search:
            cls.find_global_variables(
        ...
        cls.extract_array_info(
            ...
            cls.var_dummy[child_subroutine],
            child_subroutine,
        )
        ...
        for key, values in cls.dec_global[child_subroutine].items():
            if walk(values, F23.Function_Subprogram):
                ...
                for i, call_site in enumerate(
                    cls.call_within_sub[child_subroutine][child_function], 1
                ):
                    self.processor.logger.info(f"Call site #{i}: {call_site.tostr()}")
                ...
                if child_function in self.isolated_subroutines:
                    ...
                    self.isolate_procedure_function(
                        cls, child_subroutine, child_function
                    )
                    function_tree = cls.subroutines[child_function]
        ...
        if child_functions:
            self.processor.logger.info(
                f"Processing {len(child_functions)} function calls within subroutine {child_subroutine}"
            )
            for child_function in child_functions:
                self.collect_global_vars_decl(
                    cls.dec_global[child_function], cls.dec_global[child_subroutine]
                )

        if dummy_as_local:
            dummy_as_local = dummy_as_local.intersection(
                cls.all_array_info[child_subroutine].keys()
            )
        if dummy_as_local:
        ...
        subroutine_tree_cp = self.processor.parse_fortran_string(
            subroutine_tree.tofortran()
        )
        working_tree = self.processor.parse_fortran_string(subroutine_tree_cp)
        ...
        if not dummy_as_local:
            ...
            os.makedirs(sub_dir, exist_ok=True)
            self.processor.logger.info(
                f"{child_subroutine} directory created inside benchmark: {sub_dir}"
            )
            ...
            arg_list = ", ".join(
                [name for name in cls.dummy_arg_list[child_subroutine]]
            )
    ...
    def isolate_parent_subroutine(
        self, cls, grand_parent_subroutine, parent_subroutine, subroutines_parent=None
    ):
        assert os.path.exists(self.processor.benchmark_dir), (
            "benchmark directory does not exist!"
        )
        ...
        os.makedirs(sub_dir, exist_ok=True)
        self.processor.logger.info(
            f"{parent_subroutine} directory created inside benchmark: {sub_dir}"
        )
        ...
        while queue:
            ...
            if subroutines_parent is not None:
                for subroutine_key_parent in subroutines_parent:
                    self.child_subroutine_call[subroutine_key_parent].append(
                        child_subroutine_tree
                    )

            if child_subroutine_key not in cls.call_within_sub:
                mod_child_subroutine_tree, error_flag, child_function_tree = (
                    self.isolate_child_subroutine(
                        cls,
                        parent_subroutine,
                ...
                if mod_child_subroutine_tree is not None:
                    self.child_subroutine_call[parent_subroutine].append(
                        mod_child_subroutine_tree
                    )

                if error_flag is not None:
                    self.child_error_flag[parent_subroutine][child_subroutine_key] = (
                        error_flag
                    )
                ...
                self.collect_global_vars_decl(
                    cls.dec_global[child_subroutine_key],
                    cls.dec_global[parent_subroutine],
                )
            else:
                self.parent_subroutine_call.add(parent_subroutine)
                self.isolate_parent_subroutine(
                    cls,
                    parent_subroutine,
                    child_subroutine_key,
                    self.parent_subroutine_call,
                )

        self.processor.logger.info(
            f"Now, trying to isolate a parent subroutine .... {parent_subroutine}"
        )
        sub_trees = []

        call_statements = cls.call_within_sub[grand_parent_subroutine][
            parent_subroutine
        ]
        for i, call_stmt in enumerate(call_statements):
            self.processor.logger.info(f"  Call site {i + 1}: {call_stmt.tostr()}")
        ...
        cls.extract_intent(
            parent_subroutine, subroutine_tree, cls.call_within_sub[parent_subroutine]
        )
        ...
        cls.var_global[parent_subroutine] = [
            name
            for name in cls.var_global[parent_subroutine]
        ...
        cls.process_declaration_variables(
            cls.var_dummy[parent_subroutine], parent_subroutine
        )
        for key in cls.dec_global[parent_subroutine].keys():
            cls.process_declaration_variables(
                cls.dec_global[parent_subroutine][key], parent_subroutine
            )
        ...
        shape_to_search = [
            var
            for var in cls.shapes_variables[parent_subroutine]
            if var.string not in scalar_names and var.string not in global_names
        ]
        ...
        if subroutines_parent is not None:
            for subroutine_key_parent in subroutines_parent:
                self.collect_global_vars_decl(
                    cls.dec_global[parent_subroutine],
                    cls.dec_global[subroutine_key_parent],
                )

        cls.extract_array_info(
            cls.dec_global[parent_subroutine],
            cls.var_dummy[parent_subroutine],
            parent_subroutine,
        )
        ...
        for key, values in cls.dec_global[parent_subroutine].items():
            if walk(values, F23.Function_Subprogram):
                ...
                for i, call_site in enumerate(
                    cls.call_within_sub[parent_subroutine][child_function], 1
                ):
                    self.processor.logger.info(f"Call site #{i}: {call_site.tostr()}")
                ...
                #  function_tree = cls.subroutines[child_function]
                    ...
                    self.isolate_procedure_function(
                        cls, parent_subroutine, child_function
                    )
                #  function_tree = cls.subroutines[child_function]
        ...
        if child_functions:
            self.processor.logger.info(
                f"Processing {len(child_functions)} function calls within subroutine {parent_subroutine}"
            )
            for child_function in child_functions:
                self.collect_global_vars_decl(
                    cls.dec_global[child_function], cls.dec_global[parent_subroutine]
                )
    ...
    def process_subroutines(self):
        ...
        self.processor.logger.info(
            f"Found {len(cls.subroutine_keys_all)} total subroutines: {cls.subroutine_keys_all}"
        )
        self.processor.logger.info(
            f"Found {len(cls.subroutine_keys_ncl)} subroutines with no internal calls to any subroutine: {cls.subroutine_keys_ncl}"
        )
        self.processor.logger.info(
            f"Found {len(cls.call_within_sub)} parent subroutines that call other subroutines"
        )
        ...
        for parent_subroutine in cls.call_within_sub.keys():
            ...
            self.processor.logger.info(
                f"Parent subroutine '{parent_subroutine}' calls {len(children)} child subroutines:"
            )
            for child_subroutine, call_statements in children.items():
                self.processor.logger.info(
                    f"  - '{child_subroutine}': {len(call_statements)} call site(s)"
                )
                for i, call_stmt in enumerate(call_statements):
                    self.processor.logger.info(
                        f"    Call site {i + 1}: {call_stmt.tostr()}"
                    )

        # Process each parent subroutine and its children
        # grand_parent_subroutine = "hydrol_main"
        parent_subroutine = "explicitsnow_main"
        children = ["explicitsnow_transf"]  # cls.call_within_sub[parent_subroutine]
        self.processor.logger.info(
            f"Processing parent subroutine: '{parent_subroutine}' with {len(children)} children"
        )
        # Process each child subroutine of this parent
        for child_subroutine in children:
            self.processor.logger.info(
                f"  Isolating child subroutine: '{child_subroutine}' (called from '{parent_subroutine}')"
            )
            try:
                ...
                self.processor.logger.info(
                    f"  Successfully isolated child subroutine: '{child_subroutine}'"
                )
            except Exception as e:
                self.processor.logger.error(
                    f"  Failed to isolate child schild_subroutineubroutine '{child_subroutine}': {e}"
                )
                raise
