...
from utils import ast_walk, get_instance_name, load_code_templates, python_parser
from jax_utils import convert_np_jnp, get_class_info_from_ast, topo_sort, get_name, contains_name, \
CallEdge, RemoveLogging
...
class AutoDiff:
    ...
    def __init__(
        ...
        if logger is None:
            import warnings
            warnings.warn('No logger provided; using default ' \
            'Logger()', stacklevel=2)
            self.logger = Logger()
        else:
            self.logger = logger
        ...
        self._SHAPE_CTORS: set = {
            ...
            'rand', 'randn', 'zeros_like', 'ones_like', 'empty_like',
        }
        ...
        self.transform = self.logger.log_event(
            'Create Module'
        )(self.transform)
        self.write_to_file = self.logger.log_event(
            'Transfer to Python File'
        )(self.write_to_file)
    ...
    def _add_jax_imports(self, class_module: ast.AST, import_remove: List[str] = ['logging']) -> None:
        """
    ...
    def _prepare_class(self, class_module: ast.AST, main_details: Dict) -> Tuple:
        ...
        """
        try:
            class_defs = [node for node in ast.walk(class_module) if isinstance(node, ast.ClassDef)]
            ...
            declaration_initalization = [convert_np_jnp(fn) for fn in declaration_initalization]
            ...
            functions = [
                ...
                if node.name not in {'__init__', 'declaration_initialization', 'timer', 'wrapper'}
            ]
            ...
            jax_converter = JaxConverter(cls_info=self.cls_info, logger=self.logger, mode=self.mode)
    ...
    def _to_module(self, source: Union[str, ast.Module]) -> ast.Module:
        """
        ...
        raise TypeError(f'Expected a file path or ast.Module, got {type(source).__name__}')

    def transform(self, main_file: Union[str, ast.Module], class_file: Union[str, ast.Module]) -> None:
        """
        ...
        try:
            ...
            if isinstance(main_file, str) and isinstance(class_file, str):
                ...
                class_import = next(
                    (
                        stmt for stmt in main_module.body
                        if isinstance(stmt, ast.ImportFrom) and stmt.module
                    ),
                    None,
                )
                if class_import is None:
                    raise ValueError('Cannot derive output paths: no ImportFrom found in main_module')
            ...
            main_func = next(
                (node for node in ast_walk(main_module, ast.FunctionDef) if node.name == 'main'),
                None,
            )
            ...
            last_import_pos = [
                i for i, node in enumerate(main_module.body)
    ...
    def write_to_file(self, file_path: Union[str, Path], tree: ast.Module) -> None:
        """
    ...
    def _get_timer(self, timer_node: Optional[ast.FunctionDef] = None) -> Optional[ast.FunctionDef]:
        """
        ...
        try:
            ...
            timer_template = templates['JAX_templates']['JAX_timer_template']['template']
    ...
    def get_class_dep(self, functions: List[ast.FunctionDef]) -> Optional[Dict[str, List[CallEdge]]]:
        """
        ...
        try:
            ...
            for stmt in ast.walk(node):
                ...
                for target in stmt.targets:
                    ...
                    if (
                        ...
                        input_attr_map[target.id] = 'asarray' if attr == 'zeros' else attr

            # Build the x_d = jnp.<ctor>(x) assignment nodes
            jd_assignments = [
                ast.Assign(
                    targets=[ast.Name(id=f'{name}{self._MODE_SUFFIX[self.mode]}', ctx=ast.Store())],
                    value=ast.Call(
                        func=ast.Attribute(
                            value=ast.Name(id='jnp', ctx=ast.Load()),
            ...
            for i, item in enumerate(node.body):
                if not isinstance(item, (ast.Expr, ast.Assign)):
                    continue
                call = item.value if isinstance(item, ast.Expr) else (
                    item.value if isinstance(item, ast.Assign) else None
                )
                if not (call and isinstance(call, ast.Call) and isinstance(call.func, ast.Attribute)):
                    continue
                instance_name = call.func.value.id if isinstance(call.func.value, ast.Name) else None
            ...
            if insert_pos is None:
                raise ValueError(f'Could not find insertion point for {self._MODE_SUFFIX[self.mode]} assignments in main body')
    ...
    def modify_ast(self, node: ast.AST, class_modif: Dict) -> None:
        ...
        try:
            class Modifier(ast.NodeTransformer):
                ...
                def visit_Assign(self, node: ast.Assign) -> Union[ast.AST, List[ast.AST]]:
                    if not isinstance(node.value, ast.Call):
                    ...
                    if isinstance(call.func, ast.Name):
                        for target in node.targets:
                            if isinstance(target, ast.Name) and target.id in self.class_modif:
                    ...
                    if isinstance(call.func, ast.Attribute):
                        ...
                        if method in self.class_modif and method != 'declaration_initialization':
                ...
                def visit_Attribute(self, node: ast.Attribute) -> ast.Attribute:
                    if isinstance(node.value, ast.Name) and node.value.id in self.class_modif:
                ...
                if isinstance(value, list):
                    for item in value:
                        if isinstance(item, ast.AST):
                            self._rename_call_args(item, input_args, method_calls, test_calls)
        ...
        try:
            if isinstance(node.func, ast.Attribute):
                ...
                for call in method_calls:
                    if call.get('instance') == instance_name and call.get('method') == method_name:
                        return True
        ...
        try:
                    ...
                    if isinstance(call.func, ast.Name):
                        ...
                        if isinstance(obj, ast.Name) and obj.id in instance_to_class:
                            method_calls.append({
                                ...
                                'args_shape': self._get_args_shape(tree, self._extract_args(call)),
                            })
            ...
            for node in ast.walk(tree):
                ...
                if not already_recorded:
                    method_calls.append({
                        ...
                        'args_shape': self._get_args_shape(tree, self._extract_args(node)),
                    })
            ...
            for node in ast.walk(tree):
                ...
                test_calls.append({
                    ...
                    'args': [arg.id if isinstance(arg, ast.Name) else None for arg in node.args],
                })
            ...
            self.logger.exception(f'Exception in extract_main_content: ', e)
            raise
    ...
    def _index_functions(self, functions: List[ast.FunctionDef]) -> Dict[str, ast.FunctionDef]:
        """
        ...
        try:
            ...
            while worklist:
                ...
                for edge in graph.get(ctx.callee, []):
                    ...
                    if edge.callee not in fn_shapes or fn_shapes[edge.callee] != child_shapes:
    ...
    def _resolve_inline_shape(self, node: ast.AST) -> Optional[List]:
        """
        ...
        try:
            ...
            is_ctor = (
                ...
                isinstance(func, ast.Name) and func.id in self._SHAPE_CTORS
            )
    ...
    def _extract_shape_arg(self, node: ast.AST) -> Optional[List]:
        """
    ...
    def _extract_single_dim(self, node: ast.AST) -> Optional[Any]:
        """
    ...
    def _resolve_attr_or_subscript(
        ...
        try:
            ...
            if isinstance(node, ast.Subscript):
                ...
                if parent_name and parent_name in arg_shapes:
                    return self._infer_subscript_shape(arg_shapes[parent_name], node.slice)
    ...
    def _convert_function_body(self, node: ast.FunctionDef, converter: JaxConverter) -> None:
        """
                    ...
                    for n in ast_walk(new_tree, ast.Assign):
                        ...
                        if isinstance(n.value, (ast.Constant, ast.BinOp)):
                            n.value = ast.Call(
                                ...
                                args=[ast.Tuple(elts=vector_loop, ctx=ast.Load()), n.value],
                                keywords=[
                                    ast.keyword(
                                        arg='dtype',
                                        value=ast.Attribute(
    ...
    def _create_call_statement(
        ...
        try:
            ...
            if isinstance(ret, ast.Tuple):
                targets = [
                    ast.Name(id=f'{n.id}{self._MODE_SUFFIX[self.mode]}', ctx=ast.Store())
                    for n in ret.elts
            ...
            if isinstance(ret, ast.Name):
                return ast.Assign(
                    targets=[ast.Name(id=f'{ret.id}{self._MODE_SUFFIX[self.mode]}', ctx=ast.Store())],
                    value=call,
                )
            ...
            for _, cls_attributes in self.cls_info.items():
                ...
                for attr_info in attributes.values():
                    for dim in attr_info.get('dimensions', []):
                        if isinstance(dim, str) and self._check_dimension_static(attributes, dim):
                            arr_dim.add(dim)
                    ...
                    dynamic_static_fields.append(
                        ast.AnnAssign(
                            ...
                            value=ast.Call(
                                ...
                                args=[],
                                keywords=[ast.keyword(arg='static', value=ast.Constant(value=True))],
                            ),
                            simple=1,
                        )
                    )
    ...
    def _remove_numpy_call(
        ...
        dynamic_field: Optional[List[List[ast.AnnAssign]]] = None,
    ) -> None:
        """
        ...
        try:
            ...
            if missing:
                insert_pos = [
                    i for i, n in enumerate(init_function.body)
    ...
    def _check_read_rhs_assign_value(self, node: ast.AST) -> bool:
        ...
        try:
            ...
            if isinstance(node, ast.Call):
                return any(
                    self._check_read_rhs_assign_value(arg) for arg in node.args
                )
            return False
        except Exception as e:
    ...
    def _correct_tuple_shape(
        ...
        """
        try:
            tuple_shape.elts = [self._correct_name(elt, value) for elt in tuple_shape.elts]
            return tuple_shape
        except Exception as e:
            ...
            for assign in ast_walk(decl_init_function, ast.Assign):
                ...
                if not has_read:
                    ...
                    for i, arg in enumerate(assign.value.args):
                        if isinstance(arg, ast.Tuple):
                            assign.value.args[i] = self._correct_tuple_shape(arg, known_names)
    ...
    def _add_jax_for_file_reading(self, decl_init_function: ast.FunctionDef) -> None:
        ...
        try:
            ...
            if not for_elements:
                ...
                call_setattr = next(
                    (
                        node for node in ast_walk(decl_init_function, ast.Expr)
                        if (
                            isinstance(node.value, ast.Call)
                ...
                if call_setattr is None:
                    raise ValueError('setattr call not found in declaration_initialization')
                ...
                if parent_body is None:
                    raise ValueError('Could not locate parent body for setattr replacement')
