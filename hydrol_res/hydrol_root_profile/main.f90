...
PROGRAM main
  ...
  REAL(KIND = r_std), DIMENSION(kjpindex, nvm, nslm, nroot_prof) :: root_profile, root_profile_cpu
  REAL(KIND = r_std), DIMENSION(kjpindex, nvm, ndepths) :: root_depth, root_depth_cpu
  ...
  CALL declarations
  CALL initialization
  CALL read_dummy(altmax, sm, smw, ji, error_flag_hydrol_root_profile_1, error_flag_hydrol_root_profile_2)
  CALL hydrol_root_profile(kjpindex, altmax, sm, smw, root_profile, root_depth)
  root_profile_cpu = root_profile
  root_depth_cpu = root_depth
  CALL initialization
  CALL read_dummy(altmax, sm, smw, ji, error_flag_hydrol_root_profile_1, error_flag_hydrol_root_profile_2)
  error_flag_hydrol_root_profile_1 = 0
  error_flag_hydrol_root_profile_2 = 0
  !$ACC ENTER DATA COPYIN(altmax, sm, smw, root_profile, root_depth)
  !$ACC PARALLEL LOOP INDEPENDENT REDUCTION(+:error_flag_hydrol_root_profile_1, error_flag_hydrol_root_profile_2)
  DO ji = 1, kjpindex
  ...
  END DO
  !$ACC END PARALLEL
  !$ACC UPDATE SELF(root_profile, root_depth)
  !$ACC EXIT DATA DELETE(altmax, sm, smw, root_profile, root_depth)
  IF (ALL(root_profile .EQ. root_profile_cpu)) THEN
    WRITE(*, *) 'Test passed: All elements in root_profile_gpu are equal to root_profile_cpu.'
  ELSE
    WRITE(*, *) ''
    WRITE(*, *) 'Test failed: All elements in root_profile_gpu do not match root_profile_cpu.'
    WRITE(*, '(A, E25.16)') 'Maximum absolute error:', MAXVAL(ABS(root_profile - root_profile_cpu))
    WRITE(*, '(A, 2E25.16)') 'Min and Max of root_profile_gpu:', MINVAL(root_profile), MAXVAL(root_profile)
    WRITE(*, '(A, 2E25.16)') 'Min and Max of root_profile_cpu:', MINVAL(root_profile_cpu), MAXVAL(root_profile_cpu)
    WRITE(*, *) ''
  END IF
  IF (ALL(root_depth .EQ. root_depth_cpu)) THEN
    WRITE(*, *) 'Test passed: All elements in root_depth_gpu are equal to root_depth_cpu.'
  ELSE
    WRITE(*, *) ''
    WRITE(*, *) 'Test failed: All elements in root_depth_gpu do not match root_depth_cpu.'
    WRITE(*, '(A, E25.16)') 'Maximum absolute error:', MAXVAL(ABS(root_depth - root_depth_cpu))
    WRITE(*, '(A, 2E25.16)') 'Min and Max of root_depth_gpu:', MINVAL(root_depth), MAXVAL(root_depth)
    WRITE(*, '(A, 2E25.16)') 'Min and Max of root_depth_cpu:', MINVAL(root_depth_cpu), MAXVAL(root_depth_cpu)
    WRITE(*, *) ''
  END IF
  ...
  SUBROUTINE read_dummy(altmax, sm, smw, ji, error_flag_hydrol_root_profile_1, error_flag_hydrol_root_profile_2)
    INTEGER(KIND = i_std) :: error_flag_hydrol_root_profile_2
    INTEGER(KIND = i_std) :: error_flag_hydrol_root_profile_1
    INTEGER(KIND = i_std) :: ji
    REAL(KIND = r_std), DIMENSION(kjpindex, nslm) :: smw
