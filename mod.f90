...
SUBROUTINE explicitsnow_gone_acc(ji, kjpindex, pgflux, snowheat, snowtemp, snowdz, snowrho, snowliq, grndflux, snowmelt)
  !$ACC ROUTINE SEQ
  ...
  !! Snow melt
  REAL(KIND = r_std) :: thrufal
  !! Water leaving snowpack(kg/m2/s)

  !! 0.4 Local variables

  INTEGER(KIND = i_std) :: jj
  REAL(KIND = r_std) :: snowgone_delta
  REAL(KIND = r_std) :: totsnowheat
  !!snow heat content at each layer
  REAL(KIND = r_std) :: snowdepth_crit

  ! first caculate total snowpack snow heat content
  !snowgone_delta(:) = un
  !thrufal(:)=0.0
  !snowmelt(:)=0
  totsnowheat = SUM(snowheat(ji, :), DIM = 1)
...
END SUBROUTINE explicitsnow_gone_acc